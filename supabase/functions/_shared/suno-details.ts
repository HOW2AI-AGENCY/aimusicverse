 ...
 * across all 7 details endpoints.
 *
 * Note: `suno-check-status` was removed in Sprint 054-A7' (dead code —
 * zero client callers). Per-task polling now lives in `useSunoTaskDetails`
 * (see `src/hooks/generation/useSunoTaskDetails.ts`).
 *
 ...
 * Used by `useSunoTaskDetails` generic hook to reject unknown values early.
 */
