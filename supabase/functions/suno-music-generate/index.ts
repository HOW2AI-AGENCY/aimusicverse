...
function isRetriableModelError(errorMsg: string): boolean {
  const lowerError = errorMsg.toLowerCase();
  return lowerError.includes('model error') || lowerError.includes('audio generation failed') || lowerError.includes('malformed');
}
...
serve(async (req) => {
  ...
  try {
    ...
    const sunoTaskId = (sunoData as any)?.data?.taskId ?? (sunoData as any)?.data?.task_id ?? (sunoData as any)?.data?.id;
    ...
    logger.success('Generation started', { trackId: track.id, taskId: task.id, sunoTaskId, model: currentModel, retries: retryCount });
