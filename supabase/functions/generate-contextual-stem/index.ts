...
serve(async (req) => {
  ...
  try {
    ...
    if (!trackId || !stemType) {
      return new Response(JSON.stringify({ error: "trackId and stemType are required" }), {
        status: 400,
    ...
    if (!trackContext && trackUrl) {
      ...
      try {
        const contextResponse = await fetch(`${Deno.env.get("SUPABASE_URL")}/functions/v1/analyze-track-context`, {
          method: "POST",
          headers: {
    ...
    const bpmKey =
      trackContext?.bpm && trackContext?.key
    ...
    const prompt = template.replace("{style}", style).replace("{bpmKey}", bpmKey).replace("{hint}", hint).trim();
    ...
    if (!replicateResponse.ok) {
      ...
      return new Response(JSON.stringify({ error: "Generation service error", details: errorText }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (insertError) {
      ...
      return new Response(JSON.stringify({ error: "Failed to save generated stem", details: insertError.message }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
