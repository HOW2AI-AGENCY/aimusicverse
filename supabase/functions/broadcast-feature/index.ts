...
Откройте приложение, чтобы попробовать!`;
    ...
    // Get users to notify
    let query = supabase.from("profiles").select("telegram_chat_id, first_name").not("telegram_chat_id", "is", null);
    ...
    for (let i = 0; i < users.length; i += batchSize) {
      ...
      await Promise.all(
        batch.map(async (user) => {
          try {
            const personalizedMessage = user.first_name ? `Привет, ${user.first_name}! ${message}` : message;
