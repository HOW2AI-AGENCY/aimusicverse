  ...
  try {
    ...
    if (requestUserId && requestUserId !== user.id) {
      ...
      if (!isAdmin) {
        ...
        return new Response(JSON.stringify({ error: "Forbidden: cannot create invoice for another user" }), {
          status: 403,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
    ...
    const { error: txError } = await supabase.from("stars_transactions").insert({
      id: transactionId,
      user_id: userId,
    ...
    if (!telegramResult.ok) {
      ...
      return new Response(JSON.stringify({ error: "Failed to create invoice", details: telegramResult }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
