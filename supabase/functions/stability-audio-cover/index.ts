  ...
  try {
    ...
    if (source === "telegram_bot") {
      ...
      if (!botSecret || botSecret !== telegramBotSecret) {
        ...
        return new Response(JSON.stringify({ error: "Unauthorized bot request" }), {
          status: 401,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      ...
      if (!telegramUserId) {
        return new Response(JSON.stringify({ error: "User ID is required for telegram bot requests" }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
    ...
    if (source !== "telegram_bot") {
      ...
      if (!isAdmin) {
        ...
        if (creditsError) {
          ...
          return new Response(JSON.stringify({ error: "Ошибка проверки баланса" }), {
            status: 500,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
    ...
    if (!falResponse.ok) {
      ...
      return new Response(JSON.stringify({ error: `Stability AI error: ${errorText}` }), {
    ...
    if (!requestId) {
      ...
      return new Response(JSON.stringify({ error: "No request ID received from Stability AI" }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
...
async function pollForResult(
  ...
  if (trackId) {
    await supabase.from("tracks").update({ status: "failed", error_message: "Generation timeout" }).eq("id", trackId);
  }
}
