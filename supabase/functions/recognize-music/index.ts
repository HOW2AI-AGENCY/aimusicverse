...
serve(async (req) => {
  ...
  try {
    ...
    if (!auddApiKey) {
      ...
      return new Response(JSON.stringify({ error: "AUDD_API_KEY not configured" }), {
        status: 500,
    ...
    if (!audioBase64 && !audioUrl) {
      return new Response(JSON.stringify({ error: "Either audioBase64 or audioUrl is required" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({ error: "Music recognition failed", details: errorText }), {
    ...
    if (result.status === "error") {
      return new Response(JSON.stringify({ error: "Recognition failed", details: result }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
