...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `Replicate API error: ${replicateError instanceof Error ? replicateError.message : "Unknown error"}`,
      );
    }
    ...
    if (track_id) {
      ...
      if (Object.keys(updates).length > 0) {
        await supabase.from("tracks").update(updates).eq("id", track_id);
...
function normalizeAnalysis(analysis: Partial<AudioAnalysisResponse>): AudioAnalysisResponse {
  return {
    ...
    tempo: ["slow", "medium", "fast"].includes(analysis.tempo as string)
      ...
      : "medium",
    bpm: typeof analysis.bpm === "number" ? Math.max(60, Math.min(200, Math.round(analysis.bpm))) : 120,
    ...
    energy: {
      level: ["low", "medium", "high"].includes(analysis.energy?.level as string)
        ...
        : "medium",
      arousal:
        typeof analysis.energy?.arousal === "number"
          ? Math.max(0, Math.min(100, Math.round(analysis.energy.arousal)))
          : 50,
      valence:
        typeof analysis.energy?.valence === "number"
