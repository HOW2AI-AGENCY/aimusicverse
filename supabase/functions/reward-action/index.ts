...
const ACTION_REWARDS = {
  ...
  comment_posted: {
    credits: ECONOMY.COMMENT_POSTED.credits,
    experience: ECONOMY.COMMENT_POSTED.xp,
...
async function checkAndUnlockAchievements(supabase: any, userId: string, actionType: string, metadata?: any) {
  try {
    ...
    const [{ count: trackCount }, { data: credits }, { data: unlockedAchievements }] = await Promise.all([
      supabase
        .from("tracks")
    ...
    for (const achievement of achievementsToUnlock) {
      ...
      if (achievement.credits_reward > 0 || achievement.experience_reward > 0) {
        const { data: currentCredits } = await supabase.from("user_credits").select("*").eq("user_id", userId).single();
