...
serve(async (req) => {
  ...
  try {
    ...
    if (providedAudioUrl) {
      ...
      // as the uploadUrl for /api/v1/generate/upload-cover.
    ...
    if (customMode) {
      ...
      // Non-custom mode
      if (!prompt) {
        return new Response(JSON.stringify({ error: "Prompt is required in non-custom mode" }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
    ...
    if (!response.ok || !isSunoSuccessCode(data.code)) {
      ...
      return new Response(JSON.stringify({ error: data.msg || "Failed to cover audio" }), {
