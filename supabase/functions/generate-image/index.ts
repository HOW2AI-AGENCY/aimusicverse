...
const IMAGE_CONFIGS: Record<ImageType, ImageConfig> = {
  project_cover: {
    ...
    basePrompt: (ctx) =>
      `Create a professional music album cover. ${ctx.prompt || ""}. Style: ${ctx.genre || "modern"}, mood: ${ctx.mood || "dynamic"}. High quality, artistic, suitable for streaming platforms.`,
  ...
  track_cover: {
    ...
    dimensions: "1024x1024",
    basePrompt: (ctx) =>
      `Create a stunning single/track cover art. ${ctx.prompt || ""}. Title: "${ctx.title || "Untitled"}". Style: ${ctx.style || "modern electronic"}. Vibrant, eye-catching, professional quality.`,
    updateTable: "tracks",
    updateColumn: "cover_url",
    updateIdColumn: "id",
  },
  profile_avatar: {
    ...
    dimensions: "512x512",
    basePrompt: (ctx) =>
      `Create an artistic avatar for a music producer${ctx.displayName ? ` named "${ctx.displayName}"` : ""}. ${ctx.bio ? `About: ${ctx.bio}. ` : ""}${ctx.genres?.length ? `Genres: ${ctx.genres.join(", ")}. ` : ""}Modern, creative, vibrant colors. NO text, NO watermarks.`,
  },
  profile_banner: {
    storageBucket: "avatars",
    ...
    dimensions: "1200x400",
    basePrompt: (ctx) =>
      `Create a stunning profile banner for a music artist${ctx.displayName ? ` named "${ctx.displayName}"` : ""}. ${ctx.bio ? `About: ${ctx.bio}. ` : ""}Atmospheric, cinematic, futuristic. NO text, NO watermarks.`,
  },
  artist_portrait: {
    storageBucket: "avatars",
    aspectRatio: "1:1 square",
    dimensions: "512x512",
    basePrompt: (ctx) =>
      `Create an artistic portrait/avatar for AI music artist "${ctx.name || "Artist"}". ${ctx.style_description ? `Style: ${ctx.style_description}. ` : ""}${ctx.genre_tags?.length ? `Genres: ${ctx.genre_tags.join(", ")}. ` : ""}Creative, unique, memorable. NO text.`,
    updateTable: "artists",
  ...
  blog_cover: {
    ...
    basePrompt: (ctx) =>
      `Create a professional blog post cover image. Topic: "${ctx.title || "Music"}". ${ctx.prompt || ""}. Modern, clean, editorial style. NO text overlay.`,
  ...
  playlist_cover: {
    ...
    dimensions: "640x640",
    basePrompt: (ctx) =>
      `Create a visually striking playlist cover. Playlist: "${ctx.title || "My Playlist"}". ${ctx.description ? `Theme: ${ctx.description}. ` : ""}${ctx.genres?.length ? `Genres: ${ctx.genres.join(", ")}. ` : ""}Colorful, modern, music-themed.`,
...
serve(async (req) => {
  ...
  try {
    ...
    const { error: uploadError } = await supabase.storage.from(config.storageBucket).upload(fileName, buffer, {
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from(config.storageBucket).getPublicUrl(fileName);
