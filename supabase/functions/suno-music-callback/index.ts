...
serve(async (req) => {
  ...
  try {
    ...
    if (callbackType === 'first') {
      ...
      // Validate telegram_chat_id before attempting to send (note: group chats have negative IDs)
      if (task.telegram_chat_id != null && typeof task.telegram_chat_id === 'number' && task.telegram_chat_id !== 0 && clips.length > 0) {
