  ...
  try {
    ...
    if (callbackType === "first") {
      ...
      for (let i = 0; i < clips.length; i++) {
        ...
        const projectTrackIdFromMeta =
          rawProjectTrackId && rawProjectTrackId !== "null" && rawProjectTrackId !== "undefined"
            ? rawProjectTrackId
            : null;
            ...
            if (newTrackError || !newTrack) {
              ...
              const { data: newProject, error: projectError } = await supabase
                ...
                .insert({
                  ...
                  stems_mode: "simple",
                  tracks: studioTracks,
                  duration_seconds: Math.max(originalTrack.duration_seconds || 180, newTrack.duration_seconds || 180),
                })
                .select("id")
                .single();
      ...
      if (!isAdmin) {
        try {
          logger.info("Deducting generation credits from user (atomic RPC)", {
            userId: task.user_id,
            cost: generationCost,
            model: modelUsed,
          });
