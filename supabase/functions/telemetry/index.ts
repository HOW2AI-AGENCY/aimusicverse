  ...
  try {
    ...
    const telemetryRecord = {
      ...
      error_summary: Object.fromEntries(
        Object.entries(errors).map(([code, data]) => [code, { count: data.count, lastMessage: data.message }]),
      ),
    ...
    if (telemetryEvents.length > 0) {
      const { error: insertError } = await supabase.from("telemetry_events").insert(telemetryEvents);
