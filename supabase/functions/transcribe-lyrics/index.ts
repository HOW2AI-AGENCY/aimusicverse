...
Be very careful - if you hear ANY singing or rapping, answer VOCALS: YES`;
    ...
    const languageMatch =
      flamingoOutput.match(/LANGUAGE:\s*([^\n]+)/i) || flamingoOutput.match(/language[:\s]+([a-zA-Zа-яА-Я]+)/i);
    const language = languageMatch?.[1]?.trim() || "auto";

    const genreMatch = flamingoOutput.match(/GENRE:\s*([^\n]+)/i) || flamingoOutput.match(/genre[:\s]+([^\n,]+)/i);
    const genre = genreMatch?.[1]?.trim() || null;

    const moodMatch = flamingoOutput.match(/MOOD:\s*([^\n]+)/i) || flamingoOutput.match(/mood[:\s]+([^\n,]+)/i);
    const mood = moodMatch?.[1]?.trim() || null;

    const vocalStyleMatch =
      flamingoOutput.match(/VOCAL_STYLE:\s*([^\n]+)/i) || flamingoOutput.match(/vocal style[:\s]+([^\n]+)/i);
          ...
          const lyricsOutput = (await replicate.run(
            "zsxkib/audio-flamingo-3:2856d42f07154766b0cc0f3554fb425d7c3422ae77269264fbe0c983ac759fef",
            {
              input: {
                ...
                system_prompt:
                  "Listen carefully to the audio and transcribe ALL the lyrics/words sung in the song. Write only the lyrics, nothing else. If you cannot understand some words, write [unclear].",
    ...
    if (track_id && (lyrics || genre || mood)) {
      ...
      const { error: updateError } = await supabase.from("tracks").update(updates).eq("id", track_id);
