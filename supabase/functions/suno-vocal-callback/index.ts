...
serve(async (req) => {
  ...
  try {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (!uploadError && uploadData) {
              const { data: publicData } = supabase.storage.from("project-assets").getPublicUrl(`stems/${fileName}`);
    ...
    if (stemsToInsert.length > 0) {
      ...
      await supabase.from("tracks").update({ has_stems: true }).eq("id", track.id);
    }
    ...
    return new Response(JSON.stringify({ success: true, stems_created: stemsToInsert.length }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 200,
    });
  } catch (error: any) {
