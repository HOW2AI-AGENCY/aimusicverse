...
serve(async (req) => {
  ...
  try {
    ...
    const userLanguage = language || detectLanguage(title || description || concept || genre || mood || theme || "");
  ...
  "tracks": [
    {
      ...
      "energyLevel": 7
        ...
        try {
          ...
          const jsonMatch =
            fullProjectText.match(/```json\s*([\s\S]*?)\s*```/) || fullProjectText.match(/```\s*([\s\S]*?)\s*```/);
...
Return as JSON:
{
  ...
  "narrativeArc": "..."
        ...
        try {
          const jsonMatch =
            conceptText.match(/```json\s*([\s\S]*?)\s*```/) || conceptText.match(/```\s*([\s\S]*?)\s*```/);
...
Return as JSON array:
{
  ...
  "flowNotes": "Overall pacing and emotional journey"
        ...
        try {
          const jsonMatch =
            tracklistText.match(/```json\s*([\s\S]*?)\s*```/) || tracklistText.match(/```\s*([\s\S]*?)\s*```/);
  ...
  "suggestions": [
    {
      ...
      "idealTracks": [1, 5, 8]
        ...
        try {
          const jsonMatch =
            collabText.match(/```json\s*([\s\S]*?)\s*```/) || collabText.match(/```\s*([\s\S]*?)\s*```/);
  ...
  "improvements": [
    ...
    },
    {
      ...
      "priority": "medium"
        ...
        try {
          const jsonMatch =
            analyzeText.match(/```json\s*([\s\S]*?)\s*```/) || analyzeText.match(/```\s*([\s\S]*?)\s*```/);
