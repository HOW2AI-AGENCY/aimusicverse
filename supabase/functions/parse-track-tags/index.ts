...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Get existing tags
    const { data: existingTags } = await supabase.from("suno_meta_tags").select("tag_name");

    const existingTagNames = new Set((existingTags || []).map((t) => t.tag_name.toLowerCase()));

    // Get existing styles
    const { data: existingStyles } = await supabase.from("music_styles").select("style_name");
    ...
    if (newTags.length > 0) {
      ...
      const { error: insertError } = await supabase.from("suno_meta_tags").insert(tagsToInsert).select();
    ...
    if (newStyles.length > 0) {
      ...
      const { error: insertError } = await supabase.from("music_styles").insert(stylesToInsert).select();
    ...
    const result = {
      ...
      discoveredTags: Array.from(discoveredTags.entries())
        .sort((a, b) => b[1] - a[1])
        .slice(0, 50),
      discoveredStyles: Array.from(discoveredStyles.entries())
        .sort((a, b) => b[1] - a[1])
        .slice(0, 20),
      insertedTags,
      insertedStyles,
