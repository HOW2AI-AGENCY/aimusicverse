...
function generateMidi(
  ...
  trackEvents.push((microsecondsPerBeat >> 16) & 0xff, (microsecondsPerBeat >> 8) & 0xff, microsecondsPerBeat & 0xff);
...
serve(async (req) => {
  ...
  try {
    ...
    if (validNotes.length === 0) {
      return new Response(JSON.stringify({ error: "No valid notes found" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
