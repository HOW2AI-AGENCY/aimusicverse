...
serve(async (req) => {
  ...
  try {
    ...
    if (requestedUserId && requestedUserId !== user.id) {
      ...
      if (!adminRole) {
        return new Response(JSON.stringify({ error: "Forbidden: cannot query other users subscriptions" }), {
          status: 403,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
    ...
    if (error) {
      ...
      return new Response(JSON.stringify({ error: "Failed to fetch subscription status", details: error.message }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
