...
function getRecommendedModel(stemType?: string): ModelType {
  ...
  if (type.includes("vocal") || type.includes("voice") || type.includes("melody") || type.includes("lead")) {
    return "vocal";
  }
...
serve(async (req) => {
  ...
  try {
    ...
    const { track_id, audio_url, model_type, stem_id, stem_type, auto_select = true } = await req.json();
...
Create a musically coherent sequence of notes.`;
    ...
    const { error: uploadError } = await supabase.storage.from("project-assets").upload(filePath, midiBytes, {
