...
serve(async (req) => {
  ...
  try {
    ...
    const { track_id, audio_url, embedding_type = "msd-musicnn", update_analysis = true } = await req.json();
    ...
    if (track_id) {
      const { data: track } = await supabase.from("tracks").select("user_id").eq("id", track_id).single();
    ...
    const output = (await replicate.run("mtg/music-arousal-valence", {
      input: {
        audio: audio_url,
