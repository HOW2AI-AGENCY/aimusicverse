...
serve(async (req) => {
  ...
  try {
    ...
    if (!track_id || !cover_url) {
      return new Response(JSON.stringify({ error: "track_id and cover_url are required" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    await supabase.from("cover_thumbnails").upsert({
      track_id,
...
// ============================================================
async function generateThumbnails(trackId: string, coverUrl: string): Promise<ThumbnailResult> {
  try {
    ...
    if (
      ...
      !allowedHosts.some((h) => parsedUrl.host === h || parsedUrl.host.endsWith("." + h))
    ) {
