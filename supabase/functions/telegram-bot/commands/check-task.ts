...
export async function handleCheckTask(chatId: number, userId: number, taskId: string, messageId?: number) {
  try {
    // Get user profile
    ...
    if (!profile) {
      if (messageId) {
        await editMessageText(chatId, messageId, "❌ Пользователь не найден\\. Войдите в приложение сначала\\.", {
    ...
    if (taskError || !task) {
      if (messageId) {
        await editMessageText(chatId, messageId, "❌ Задача не найдена", {
          inline_keyboard: [[{ text: "⬅️ Назад", callback_data: "status" }]],
        });
      }
      return;
    ...
    if (!task.suno_task_id) {
      if (messageId) {
        await editMessageText(chatId, messageId, "❌ Задача ещё не начала генерацию в Suno API", {
          inline_keyboard: [[{ text: "⬅️ Назад", callback_data: "status" }]],
        });
      }
      return;
    ...
    if (taskData.response?.sunoData && taskData.response.sunoData.length > 0) {
      ...
      if (taskData.status === "SUCCESS" && task.status !== "completed") {
        ...
        if (task.track_id) {
          trackButtons.push([{ text: "🎵 Прослушать", callback_data: `track_details_${task.track_id}` }]);
        }
      }
    }
