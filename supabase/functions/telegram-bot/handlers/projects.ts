...
export async function handleProjectsCallback(
  ...
  const { data: projects, count } = await supabase
    .from("music_projects")
    .select(
      "id, title, description, status, project_type, cover_url, genre, mood, total_tracks_count, approved_tracks_count",
      { count: "exact" },
    )
 ...
 */
export async function handleProjectDetails(chatId: number, projectId: string, messageId: number, callbackId: string) {
  const { data: project, error } = await supabase.from("music_projects").select("*").eq("id", projectId).single();
 ...
 */
export async function handleProjectShare(chatId: number, projectId: string, messageId: number, callbackId: string) {
 ...
 */
export async function handleProjectStats(chatId: number, projectId: string, messageId: number, callbackId: string) {
  const { data: project } = await supabase
    .from("music_projects")
  ...
  if (trackIds.length > 0) {
    const { data: tracks } = await supabase.from("tracks").select("play_count, likes_count").in("id", trackIds);
...
export async function handleProjectEdit(
  ...
  const { data: project } = await supabase.from("music_projects").select("*").eq("id", projectId).single();
...
export async function handleProjectStatusChange(
  ...
  const { data: project } = await supabase.from("music_projects").select("user_id").eq("id", projectId).single();
...
export async function handleProjectDelete(
  ...
  const { data: project } = await supabase.from("music_projects").select("user_id, title").eq("id", projectId).single();
 ...
 */
export async function handleProjectTracks(chatId: number, projectId: string, messageId: number, callbackId: string) {
  const { data: project } = await supabase.from("music_projects").select("title").eq("id", projectId).single();
...
export async function handleProjectsCarousel(
  ...
  const { data: projects, count } = await supabase
    .from("music_projects")
    .select(
      "id, title, description, status, project_type, cover_url, genre, mood, total_tracks_count, approved_tracks_count, created_at",
      { count: "exact" },
    )
    .eq("user_id", profile.user_id)
    .order("updated_at", { ascending: false });
