...
export async function getMenuItem(menuKey: string): Promise<MenuItem | null> {
  ...
  const { data, error } = await supabase.from("telegram_menu_items").select("*").eq("menu_key", menuKey).single();
...
function getMenuImageUrl(item: MenuItem): string {
  return item.image_url || item.image_fallback || DEFAULT_IMAGES[item.menu_key] || DEFAULT_IMAGES.main;
}
...
export async function handleSubmenu(
  ...
  await deleteAndSendNewMenuPhoto(chatId, userId, image, caption, { inline_keyboard: keyboard }, menuKey);
