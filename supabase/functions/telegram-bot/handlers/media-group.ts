...
export async function sendTrackBatch(
  ...
  const sendPromises = tracks.map(async (track, index) => {
    try {
      const caption = index === 0 && options.caption ? options.caption : `🎵 *${escapeMarkdownV2(track.title)}*`;
 ...
 */
async function sendAudioWithMetadata(chatId: number, track: TrackInfo, caption: string): Promise<any> {
 ...
 */
export function addToMediaGroup(userId: number, chatId: number, file: MediaGroupFile): void {
 ...
 */
async function storeMediaGroupSession(userId: number, files: MediaGroupFile[]): Promise<void> {
  try {
...
async function processUploadAction(
  ...
  const { error: uploadError } = await supabase.storage.from("reference-audio").upload(storagePath, arrayBuffer, {
