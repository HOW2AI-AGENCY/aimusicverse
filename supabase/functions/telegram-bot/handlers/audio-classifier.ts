 ...
 */
export async function setPendingClassification(telegramUserId: number, data: PendingClassification): Promise<void> {
  try {
 ...
 */
export async function getPendingClassification(telegramUserId: number): Promise<PendingClassification | null> {
  try {
 ...
 */
export async function consumePendingClassification(telegramUserId: number): Promise<PendingClassification | null> {
  try {
...
function buildUploadedActionKeyboard(referenceId: string, skipStems: boolean): InlineButton[][] {
  ...
  // Analyze button
  rows.push([{ text: "🔍 Анализировать стиль", callback_data: `audio_action_analyze_${referenceId}` }]);

  // Stems button (only for full tracks)
  if (!skipStems) {
    rows.push([{ text: "🎛 Разделить на стемы", callback_data: `audio_action_stems_${referenceId}` }]);
  }

  // Both actions
  if (!skipStems) {
    rows.push([{ text: "✨ Анализ + Стемы", callback_data: `audio_action_full_${referenceId}` }]);
  }
  ...
  if (data.startsWith("audio_action_")) {
    ...
    if (pipelineResult?.success) {
      ...
      if (analysis && runAnalysis) {
        ...
        if (analysis.genre)
          successText += `• Жанр: ${escapeMarkdown(analysis.genre)}${analysis.sub_genre ? ` \\(${escapeMarkdown(analysis.sub_genre)}\\)` : ""}\n`;
        ...
        if (analysis.tempo || analysis.bpm_estimate)
          successText += `• Темп: ${escapeMarkdown(analysis.tempo || "")}${analysis.bpm_estimate ? ` \\(~${analysis.bpm_estimate} BPM\\)` : ""}\n`;
            ...
            [
              {
                ...
                web_app: { url: `${BOT_CONFIG.miniAppUrl}/reference/${referenceId}` },
              },
            ],
          ],
        },
 ...
 */
function buildCompletedActionKeyboard(
  referenceId: string,
  ...
  // Back to main menu
  rows.push([{ text: "🏠 Главное меню", callback_data: "menu_main" }]);
