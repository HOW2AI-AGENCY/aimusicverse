...
export async function handleQuickActions(chatId: number, userId: number, messageId?: number): Promise<void> {
...
export async function handleConfirmQuickGen(chatId: number, userId: number, presetId: string): Promise<void> {
  const preset = getPresetById(presetId);
  if (!preset) {
    await sendMessage(chatId, "❌ Пресет не найден", undefined, null);
    return;
