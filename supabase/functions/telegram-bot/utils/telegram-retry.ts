...
export function isRetryableError(error: unknown, config: RetryConfig = {}): boolean {
  ...
  if (error instanceof Error) {
    ...
    if (message.includes("too many requests") || message.includes("retry_after") || message.includes("flood_wait")) {
      return true;
    }
  }
...
export async function storeFailedNotification(
  ...
  try {
    ...
    const { error } = await supabase.from("failed_telegram_notifications").insert({
      chat_id: chatId,
      method,
      payload,
...
export async function markNotificationSuccess(notificationId: string): Promise<void> {
  try {
    ...
    await supabase.from("failed_telegram_notifications").update({ status: "success" }).eq("id", notificationId);
  } catch (error) {
 ...
 */
export async function scheduleRetry(notificationId: string, retryCount: number, errorMessage: string): Promise<void> {
  try {
    const supabase = getSupabaseClient();
 ...
 */
export async function getPendingRetries(batchSize: number = 10): Promise<
  Array<{
    id: string;
    chat_id: number;
    method: string;
...
> {
  try {
    ...
    const { data, error } = await supabase.rpc("get_pending_telegram_retries", { batch_size: batchSize });
