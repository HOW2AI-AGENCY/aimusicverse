...
export function smartTruncate(
  ...
  const { ellipsis = "...", breakWords = false, preserveMarkdown = false } = options || {};
 ...
 */
export function highlightText(text: string, query: string, style: "bold" | "italic" | "code" = "bold"): string {
...
export function cleanSunoTags(text: string): string {
  return text
    .replace(
      /\[(?:Verse|Chorus|Bridge|Outro|Intro|Hook|Pre-Chorus|Post-Chorus|Instrumental|Break|Interlude|Refrain|End|Fade|Solo|Tag)[^\]]*\]/gi,
      "",
    )
    .replace(/\n{3,}/g, "\n\n")
    .trim();
 ...
 * Format list items
 */
export function formatList(items: string[], style: "bullet" | "numbered" | "checkmark" = "bullet"): string {
  return items
    .map((item, index) => {
      let prefix = "";
      switch (style) {
 ...
 * Create a text table
 */
export function createTable(headers: string[], rows: string[][], options?: { alignNumbers?: boolean }): string {
