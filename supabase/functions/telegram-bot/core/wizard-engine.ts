...
export class WizardEngine {
   ...
   */
  async handleCallback(chatId: number, userId: number, callbackData: string): Promise<boolean> {
    try {
   ...
   */
  async handleInput(chatId: number, userId: number, wizardType: string, input: string): Promise<boolean> {
    try {
      const state = await this.getState(userId, wizardType);
   ...
   * Cancel a wizard
   */
  async cancelWizard(chatId: number, userId: number, wizardType: string): Promise<boolean> {
    try {
      const state = await this.getState(userId, wizardType);
      if (!state) return false;
  ...
  private async saveState(state: WizardState): Promise<void> {
    try {
      ...
      const { error } = await supabase.from("telegram_wizard_state").upsert({
