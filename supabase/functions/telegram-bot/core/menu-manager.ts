...
export class MenuManager {
   ...
   */
  async showMenu(chatId: number, userId: number, menu: Menu): Promise<number | null> {
    try {
      ...
      // Update state
      state = await this.pushToStack(userId, chatId, messageId, menu.id, menu.context);
   ...
   */
  async updateMenu(chatId: number, messageId: number, menu: Partial<Menu>): Promise<boolean> {
    try {
   ...
   */
  async navigateBack(chatId: number, userId: number): Promise<boolean> {
    try {
      let state = await this.getState(userId, chatId);
   ...
   */
  async cleanup(userId: number, chatId: number): Promise<void> {
    try {
   ...
   */
  async getCurrentContext(userId: number, chatId: number): Promise<MenuContext | null> {
    try {
      const state = await this.getState(userId, chatId);
   ...
   */
  async getBreadcrumb(userId: number, chatId: number): Promise<string> {
    try {
      ...
      const breadcrumb = context.path.map((p) => pathNames[p] || p).join(" → ");
  ...
  private async clearState(userId: number): Promise<void> {
    ...
    if (profile) {
      await supabase.from("telegram_menu_state").delete().eq("user_id", profile.user_id);
    }
  }
