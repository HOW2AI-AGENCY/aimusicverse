...
function formatSocialNotification(
  ...
  if (actionUrl && !replyMarkup) {
    ...
    replyMarkup = {
      inline_keyboard: [[{ text: "🎵 Открыть", url: `${miniAppUrl}?startapp=${encodeURIComponent(actionUrl)}` }]],
    };
  }
...
serve(async (req) => {
  ...
  try {
    ...
    if (checkCircuitBreaker()) {
      ...
      return new Response(JSON.stringify({ success: false, error: "Circuit breaker open" }), {
    ...
    if (onlyId) {
      const { data, error } = await supabase.from("telegram_notification_queue").select("*").eq("id", onlyId).single();
