...
serve(async (req) => {
  ...
  try {
    ...
    if (!subscriptionId) {
      return new Response(JSON.stringify({ success: false, error: "Subscription ID required" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 400,
      });
    }
    ...
    if (subscription.status === "cancelled") {
      return new Response(JSON.stringify({ success: false, error: "Subscription already cancelled" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 400,
      });
    }
    ...
    if (updateError) {
      ...
      return new Response(JSON.stringify({ success: false, error: "Failed to cancel subscription" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 500,
      });
    }
