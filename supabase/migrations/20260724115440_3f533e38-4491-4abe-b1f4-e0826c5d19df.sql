...
BEGIN
  ...
  LOOP
    audio := COALESCE(
      NULLIF(clip->>'sourceAudioUrl',''), NULLIF(clip->>'source_audio_url',''),
      NULLIF(clip->>'audioUrl',''), NULLIF(clip->>'audio_url','')
    );
    cover := COALESCE(
      NULLIF(clip->>'sourceImageUrl',''), NULLIF(clip->>'source_image_url',''),
      NULLIF(clip->>'imageUrl',''), NULLIF(clip->>'image_url','')
    );
    duration_val := CASE
    ...
    SELECT id INTO existing_id FROM public.track_versions
      WHERE track_id = task_row.track_id AND clip_index = clip_idx LIMIT 1;
    ...
    IF existing_id IS NULL THEN
      ...
      VALUES (
        ...
        audio, cover, duration_val, clip_idx = 0, 'original', 'generated',
    ...
    ELSE
      UPDATE public.track_versions
        SET audio_url = COALESCE(NULLIF(audio_url,''), audio),
            ...
            metadata = COALESCE(metadata,'{}'::jsonb) || jsonb_build_object('rebuilt_at', now())
  ...
  SELECT id INTO first_ready FROM public.track_versions
...
-- Rerun for the broken track
SELECT * FROM public.rebuild_track_versions_from_task('e6eb9559-c976-40c9-9937-e9128683a0a1'::uuid, NULL);