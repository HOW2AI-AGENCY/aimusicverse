-- Migration: Create follows table for following system
...
CREATE INDEX IF NOT EXISTS idx_follows_status ON public.follows(status) WHERE status = 'active';
...
COMMENT ON COLUMN public.follows.status IS 'Follow status: active or blocked';
