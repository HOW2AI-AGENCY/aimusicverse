WITH normalized_tasks AS (
  SELECT
    ...
    gt.user_id,
    gt.suno_task_id,
    CASE WHEN jsonb_typeof(gt.audio_clips) = 'array' THEN gt.audio_clips ELSE '[]'::jsonb END AS clips_json,
    t.active_version_id,
  ...
  WHERE jsonb_array_length(nt.clips_json) > 0
    AND (
      ...
      OR NOT EXISTS (SELECT 1 FROM public.track_versions tv WHERE tv.track_id = nt.track_id)
    )
), extracted_clips AS (
  SELECT
    ct.task_id,
    ...
    CASE clip_item.ordinality::integer WHEN 1 THEN 'A' WHEN 2 THEN 'B' WHEN 3 THEN 'C' WHEN 4 THEN 'D' WHEN 5 THEN 'E' ELSE 'V' || clip_item.ordinality::text END AS version_label,
    COALESCE(clip_item.value->>'sourceAudioUrl', clip_item.value->>'source_audio_url', clip_item.value->>'audioUrl', clip_item.value->>'audio_url') AS audio_url,
    COALESCE(clip_item.value->>'sourceImageUrl', clip_item.value->>'source_image_url', clip_item.value->>'imageUrl', clip_item.value->>'image_url') AS cover_url,
    COALESCE(clip_item.value->>'sourceStreamAudioUrl', clip_item.value->>'source_stream_audio_url', clip_item.value->>'streamAudioUrl', clip_item.value->>'stream_audio_url', clip_item.value->>'sourceAudioUrl', clip_item.value->>'source_audio_url', clip_item.value->>'audioUrl', clip_item.value->>'audio_url') AS streaming_url,
    ...
    CASE WHEN (clip_item.value->>'duration') ~ '^[0-9]+(\.[0-9]+)?$' THEN round((clip_item.value->>'duration')::numeric)::integer ELSE NULL END AS duration_seconds
  ...
  SELECT * FROM extracted_clips WHERE audio_url IS NOT NULL
), inserted_versions AS (
  INSERT INTO public.track_versions (track_id, version_label, clip_index, version_type, is_primary, audio_url, cover_url, duration_seconds, metadata)
  SELECT
    uc.track_id,
    uc.version_label,
    uc.clip_index,
    ...
    uc.cover_url,
    uc.duration_seconds,
    jsonb_build_object('suno_id', uc.suno_id, 'suno_task_id', uc.suno_task_id, 'title', uc.clip_title, 'tags', uc.clip_tags, 'lyrics', uc.clip_lyrics, 'model_name', uc.model_name, 'backfilled', true, 'backfilled_at', now())
  FROM usable_clips uc
  WHERE NOT EXISTS (SELECT 1 FROM public.track_versions tv WHERE tv.track_id = uc.track_id AND tv.version_label = uc.version_label)
  RETURNING id, track_id, version_label, clip_index
), primary_versions AS (
  SELECT DISTINCT ON (tv.track_id) tv.track_id, tv.id AS version_id
  ...
  SELECT DISTINCT ON (track_id)
    track_id, audio_url, streaming_url, cover_url, clip_title, clip_tags, clip_lyrics, model_name, suno_id, suno_task_id, duration_seconds
  FROM usable_clips
...
WHERE t.id = f.track_id
  AND (t.audio_url IS NULL OR t.active_version_id IS NULL OR t.cover_url IS NULL);