...
- Suno replace-section requests must send replacement lyrics in provider `prompt`, style in `tags`, and modified complete lyrics in `fullLyrics`.
- Suno replace-section callbacks must persist the modified complete lyrics/tags on each created version and the active track row.
...
- Stem separation recovery lives in `sync-stale-tasks`: stale `stem_separation_tasks` must be polled from the provider, healed into `track_stems`, and marked completed/failed.
