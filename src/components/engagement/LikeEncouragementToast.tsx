...
export function LikeEncouragementProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      {children}
      <AnimatePresence>
        {currentMessage && <EncouragementToast message={currentMessage} onDismiss={handleDismiss} />}
      </AnimatePresence>
    </>
  );
}

function EncouragementToast({ message, onDismiss }: { message: EncouragementMessage; onDismiss: () => void }) {
    ...
    >
      <div className="relative p-4 rounded-xl bg-card/95 backdrop-blur-md border border-border/50 shadow-lg">
        ...
        <div className="flex items-start gap-3 pr-6">
          <div className="p-2 rounded-full bg-primary/10 flex-shrink-0">{message.icon}</div>
          <div>
