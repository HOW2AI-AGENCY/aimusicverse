...
export const TierComparisonCard = memo(function TierComparisonCard({
  ...
  const renderValue = (value: boolean | string, tierIndex: number) => {
    if (typeof value === "string") {
      return (
        <span className={cn("text-xs font-medium", tierIndex === 0 ? "text-muted-foreground" : "text-foreground")}>
          {value}
        </span>
      );
    }
  ...
  return (
    <div className={cn("overflow-x-auto -mx-4 px-4", className)}>
      <table className="w-full min-w-[320px] text-sm">
        ...
        <thead>
          <tr>
            <th className="text-left py-3 px-2 text-muted-foreground font-medium">Функции</th>
            {TIERS.map((tier, idx) => {
              ...
              return (
                <th key={tier.id} className="py-3 px-2 text-center">
                  ...
                  >
                    ...
                    <span className={cn("font-bold", tier.color)}>{tier.name}</span>
        ...
        <tbody>
          {TIER_FEATURES.map((feature, featureIdx) => (
            <motion.tr
              key={feature.name}
              className={cn("border-t border-border/50", featureIdx % 2 === 0 && "bg-muted/20")}
              initial={{ opacity: 0, x: -10 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ delay: featureIdx * 0.05 }}
            >
              <td className="py-3 px-2 text-sm text-muted-foreground">{feature.name}</td>
              <td className="py-3 px-2 text-center">{renderValue(feature.free, 0)}</td>
              <td className={cn("py-3 px-2 text-center", highlightTier === "pro" && "bg-amber-500/5")}>
                {renderValue(feature.pro, 1)}
              </td>
              <td className={cn("py-3 px-2 text-center", highlightTier === "premium" && "bg-purple-500/5")}>
