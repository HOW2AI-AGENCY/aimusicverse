...
export function ProfileBio({ bio, maxLength = 150 }: ProfileBioProps) {
  ...
  const displayBio = !shouldTruncate || isExpanded ? bio : `${bio.slice(0, maxLength)}...`;

  return (
    <div className="space-y-2">
      <p className="whitespace-pre-wrap text-sm leading-relaxed text-foreground">{displayBio}</p>
      {shouldTruncate && (
        <Button
          variant="ghost"
          size="sm"
