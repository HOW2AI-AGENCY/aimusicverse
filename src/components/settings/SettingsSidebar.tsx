    ...
    <nav className={cn("space-y-1", className)}>
      {settingsTabs.map((tab) => {
        ...
        return (
          ...
          >
            <Icon className={cn("w-5 h-5 mr-3 flex-shrink-0", isActive ? "text-primary" : "text-muted-foreground")} />
            <div className="flex-1 text-left">
              <div className={cn("text-sm font-medium", isActive && "text-primary")}>{tab.label}</div>
