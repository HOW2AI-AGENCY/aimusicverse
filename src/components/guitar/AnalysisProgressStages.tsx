...
export function AnalysisProgressStages({ currentStage, progress = 0, error, className }: AnalysisProgressStagesProps) {
  const currentStageIndex = stages.findIndex((s) => s.id === currentStage);
  const completedStages = currentStageIndex;
  const totalStages = stages.length;

  // Calculate overall progress
  const overallProgress = currentStage === "complete" ? 100 : ((completedStages + progress / 100) / totalStages) * 100;

  // Calculate time remaining
  const timeRemaining =
    currentStageIndex >= 0 ? stages.slice(currentStageIndex).reduce((sum, s) => sum + s.estimatedTime, 0) : 0;
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            {currentStage === "complete" ? (
              <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="p-2 rounded-lg bg-green-500/10">
            ...
            <div>
              <h3 className="font-semibold">
                {currentStage === "complete" ? "Анализ завершён" : error ? "Ошибка анализа" : "Анализ трека..."}
              </h3>
          ...
          <Badge
            variant={currentStage === "complete" ? "default" : error ? "destructive" : "secondary"}
            className="text-xs"
          >
            {currentStage === "complete" ? "100%" : error ? "Ошибка" : `${Math.round(overallProgress)}%`}
          </Badge>
        </div>
          ...
          return (
            <motion.div
              ...
              className={cn(
                ...
                isActive && "bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border-blue-500/30",
                isCompleted && "bg-gradient-to-r from-green-500/5 to-emerald-500/5 border-green-500/20",
            ...
            >
              ...
              <div className="relative">
                {isCompleted ? (
                  ...
                  <motion.div
                    ...
                    className={cn("p-3 rounded-full", `bg-${stage.color}-500/10`)}
                  >
                    <Icon className={cn("w-6 h-6", `text-${stage.color}-400`)} />
                  </motion.div>
                ) : (
              ...
              <div className="text-right">
                <div className="flex items-center gap-1 text-xs text-muted-foreground">
                  ...
                  {isCompleted ? <span className="text-green-400">✓</span> : <span>~{stage.estimatedTime}с</span>}
                </div>
              </div>
            </motion.div>
          );
        ...
        {currentStage === "complete" && (
          ...
          >
            <div className="flex items-center gap-3">
              <CheckCircle2 className="w-6 h-6 text-green-400" />
              <div>
                <p className="font-semibold text-sm text-green-400">Анализ успешно завершён!</p>
                <p className="text-xs text-muted-foreground">Результаты готовы к просмотру и экспорту</p>
              </div>
            </div>
          </motion.div>
        )}
