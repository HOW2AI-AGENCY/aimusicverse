...
export function ChordDiagram({ chord, className, size = "md" }: ChordDiagramProps) {
  ...
  if (!fingering) {
    ...
    return (
      <div
        className={cn("flex items-center justify-center rounded-lg bg-muted/50 border border-border", className)}
        style={{ width, height }}
      >
  ...
  return (
    <div className={cn("relative", className)}>
      {/* Chord name */}
      <div className="text-center font-mono font-bold mb-1" style={{ fontSize: fontSize + 2 }}>
        {chord}
      </div>
      ...
      <svg width={width} height={height - 15} viewBox={`0 0 ${width} ${height - 15}`}>
        ...
        {/* Base fret indicator */}
        {baseFret > 1 && (
          <text x={2} y={startY + fretSpacing / 2} fontSize={fontSize - 1} className="fill-muted-foreground">
            {baseFret}
          </text>
        )}
        ...
        {fingering.map((fret, stringIndex) => {
          ...
          return (
            <circle key={`pos-${stringIndex}`} cx={x} cy={y} r={dotSize} fill="currentColor" className="text-primary" />
          );
        })}
      </svg>
    </div>
