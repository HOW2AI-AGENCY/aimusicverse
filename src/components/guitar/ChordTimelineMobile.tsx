...
export function ChordTimelineMobile({ chords, duration, currentTime, onSeek, className }: ChordTimelineMobileProps) {
  ...
  // Find current chord
  useEffect(() => {
    const index = chords.findIndex((c) => currentTime >= c.startTime && currentTime < c.endTime);
  ...
  const navigateChord = useCallback(
    (direction: "prev" | "next") => {
      selectionChanged();
      const newIndex =
        direction === "prev" ? Math.max(0, activeChordIndex - 1) : Math.min(chords.length - 1, activeChordIndex + 1);
    ...
    <div className={cn("space-y-3", className)}>
      ...
      >
        ...
        >
          ...
          <p className="text-xs text-muted-foreground mt-1">Нажмите для просмотра диаграммы</p>
        </motion.div>
      ...
      <AnimatePresence>
        {showDiagram && activeChord && (
          ...
          >
            ...
            >
              <ChordDiagramEnhanced chord={activeChord.chord} size="lg" showFingers animated />
              <p className="text-center text-sm text-muted-foreground mt-4">Нажмите вне диаграммы для закрытия</p>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
