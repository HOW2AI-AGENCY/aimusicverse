...
export function ChordDiagramEnhanced({
  ...
  return (
    ...
    >
      {/* Chord name */}
      <div
        className={cn("text-center font-mono font-bold mb-1", isActive && "text-primary")}
      ...
      <svg width={width} height={height - 20} viewBox={`0 0 ${width} ${height - 20}`}>
        ...
        {/* Base fret indicator */}
        {baseFret > 1 && (
          <text x={4} y={startY + fretSpacing / 2 + 4} fontSize={labelSize} className="fill-muted-foreground font-mono">
            {baseFret}
          </text>
        )}
        ...
        {fingering.map((fret, stringIndex) => {
          ...
          return (
            <g key={`pos-${stringIndex}`}>
              ...
              {isActive && (
                <circle cx={x} cy={y} r={dotSize + 4} fill="hsl(var(--primary) / 0.3)" filter={`url(#glow-${chord})`} />
              )}
              <motion.circle
                cx={x}
                cy={y}
