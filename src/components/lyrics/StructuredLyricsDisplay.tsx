          ...
          <TooltipProvider key={idx}>
            <Tooltip>
              <TooltipTrigger asChild>
                <Badge variant="outline" className={cn("text-xs px-2 py-0.5", TAG_TYPE_COLORS[tag.type])}>
                  {tag.format === "bracket" ? "[" : "("}
                  {tag.value}
                  {tag.format === "bracket" ? "]" : ")"}
                </Badge>
              ...
              <TooltipContent>
                <p className="text-xs">{TAG_TYPE_INFO[tag.type].description}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        ))}
  ...
  const renderSection = (section: LyricsSection) => {
    ...
    return (
      <div key={section.id} className="mb-6">
        <Badge
          variant="outline"
          className={cn("mb-2 px-3 py-1 text-sm font-medium", getSectionTypeColor(section.type))}
        >
          {section.name}
        </Badge>
    ...
    return (
      <Card className={cn("p-6", className)}>
        <div className="flex flex-col items-center justify-center text-center py-8">
          ...
          <p className="text-muted-foreground text-sm">Текст песни отсутствует</p>
        </div>
      </Card>
    );
  }
    ...
    <Card className={cn("overflow-hidden", className)}>
      ...
      {showMetadata && (title || style) && (
        <div className="p-4 border-b bg-muted/30">
          ...
          {style && <p className="text-sm text-muted-foreground">{style}</p>}
        </div>
      )}
      ...
      {parsed.warnings.length > 0 && (
        <div className="px-4 pt-3">
          <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-3">
            <p className="text-xs font-medium text-yellow-600 dark:text-yellow-500 mb-1">⚠️ Предупреждения:</p>
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as typeof activeTab)} className="w-full">
        ...
        <TabsContent value="raw" className="m-0">
          <ScrollArea className="h-[500px]">
            <div className="p-4">
              <pre className="text-sm leading-relaxed whitespace-pre-wrap font-mono">{lyrics}</pre>
            </div>
          </ScrollArea>
        </TabsContent>
                ...
                return (
                  <div key={type}>
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-1">
                      {tagList.map((tag, idx) => (
                        <Badge key={idx} variant="outline" className={cn("text-xs", TAG_TYPE_COLORS[tagType])}>
                          {tag}
                        </Badge>
                      ))}
                    </div>
                    <p className="text-xs text-muted-foreground/60 italic mt-1">Примеры: {info.examples}</p>
                  </div>
                );
              })}
              ...
              <div className="bg-gradient-to-r from-primary/5 to-primary/10 rounded-lg p-4 border border-primary/20">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span
                    className={cn(
                      "text-lg font-bold",
                      proAnalysis.qualityScore >= 80
                        ? "text-green-500"
                        : proAnalysis.qualityScore >= 60
                          ? "text-yellow-500"
                          : "text-red-500",
                    )}
                  >
              ...
              <div className="bg-muted/30 rounded-lg p-4 border">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  ...
                  <div className="bg-background/50 p-2 rounded">
                    ...
                    <span className="ml-2 font-semibold text-green-500">{proAnalysis.syllableStats.optimal} строк</span>
                  </div>
                  <div className="bg-background/50 p-2 rounded">
                ...
                <p className="text-xs text-muted-foreground mt-2">💡 Оптимум: 6-12 слогов на строку</p>
              </div>
      ...
      <div className="p-4 border-t bg-muted/30">
        <div className="flex items-center justify-between">
          <div className="text-xs text-muted-foreground">
            {parsed.sections.length} секций •{" "}
            {parsed.tags.structural.length +
              parsed.tags.vocal.length +
              parsed.tags.dynamic.length +
              parsed.tags.instrumental.length +
              parsed.tags.emotional.length}{" "}
            тегов
          </div>
