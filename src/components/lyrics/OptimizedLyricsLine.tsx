...
export const OptimizedLyricsLine = memo(
  function OptimizedLyricsLine({
    line,
    index,
    isActive,
    isNext,
    ...
    // Empty line
    if (!line.text.trim()) {
      return <div className={cn("h-6", className)} onClick={handleClick} />;
    }
    ...
    return (
      <div
        className={cn(
          ...
          isActive && ["bg-primary/10 text-foreground font-medium", "scale-[1.02] shadow-sm"],
      ...
      >
        ...
        {/* Line text */}
        <span className={cn("flex-1 text-sm leading-relaxed", isActive && "text-primary")}>{line.text}</span>

        {/* Active indicator */}
        {isActive && <div className="w-1.5 h-1.5 rounded-full bg-primary animate-pulse flex-shrink-0" />}
      </div>
    );
  },
  (prevProps, nextProps) => {
