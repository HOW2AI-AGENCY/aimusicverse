  ...
  componentDidCatch(error: Error, errorInfo: React.ErrorInfo) {
    ...
    try {
      ...
      if (errorInfo.componentStack) {
        existing.push(
          `[${new Date().toISOString()}] [ErrorBoundary] ComponentStack: ${errorInfo.componentStack.substring(0, 300)}`,
        );
      }
      sessionStorage.setItem("musicverse_boot_log", JSON.stringify(existing));
    } catch (e) {
        ...
        <div className="min-h-screen flex items-center justify-center bg-background p-4">
          <Card className="max-w-md w-full p-6 border-destructive/20">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-muted-foreground mb-6">Произошла ошибка при загрузке приложения.</p>
