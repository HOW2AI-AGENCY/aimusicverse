...
export function AnalyzeToolPanel({ context, onExecute, onClose, isLoading }: ToolPanelProps) {
      ...
      <div className="p-3 space-y-3">
        ...
        {!hasContent ? (
          <div className="p-3 rounded-lg bg-muted/50 border border-border/50">
            <p className="text-xs text-muted-foreground text-center">Добавьте текст для анализа</p>
          </div>
        ) : (
          <>
            {/* Context Info */}
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              {context.selectedSection ? (
                <span>
                  Анализ секции: <span className="font-medium text-foreground">{context.selectedSection.type}</span>
                </span>
              ) : (
                <span>
                  Анализ всего текста:{" "}
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isSelected ? "border-purple-500/50 bg-purple-500/10" : "border-border/50 hover:bg-muted/50",
                      "disabled:opacity-50",
                    )}
                    disabled={isLoading}
                  >
