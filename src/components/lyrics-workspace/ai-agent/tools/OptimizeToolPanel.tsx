...
export function OptimizeToolPanel({ context, onExecute, onClose, isLoading }: ToolPanelProps) {
      ...
      <div className="p-3 space-y-3">
        ...
        {!hasContent ? (
          <div className="p-3 rounded-lg bg-muted/50 border border-border/50">
            <p className="text-xs text-muted-foreground text-center">Добавьте текст для оптимизации</p>
          </div>
        ) : (
          <>
            {/* Status Indicators */}
            <div className="space-y-2">
              ...
              <div
                className={cn(
                  "flex items-center gap-2 text-xs p-2 rounded-lg",
                  !context.stylePrompt ? "bg-amber-500/10 text-amber-500" : "bg-green-500/10 text-green-500",
                )}
              >
                {!context.stylePrompt ? (
                  <AlertTriangle className="w-3.5 h-3.5" />
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isSelected ? "border-primary/50 bg-primary/10" : "border-border/50 hover:bg-muted/50",
                      "disabled:opacity-50",
                    )}
                    disabled={isLoading}
                  >
            ...
            {/* Execute Button */}
            <Button onClick={handleOptimize} disabled={isLoading || selectedOptions.length === 0} className="w-full">
