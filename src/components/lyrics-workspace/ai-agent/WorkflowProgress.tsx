  ...
  return (
    ...
    >
          ...
          return (
            <div key={index} className="flex items-center">
              ...
              {index < workflow.steps.length - 1 && (
                <div className={cn("w-4 h-0.5 mx-0.5", index < currentStepIndex ? "bg-emerald-500/50" : "bg-border")} />
              )}
            </div>
          );
        })}
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-muted-foreground w-10 text-right">{Math.round(progress * 100)}%</span>
      </div>

      {/* Current step info */}
      {status === "running" && workflow.steps[currentStepIndex] && (
        <div className="mt-2 flex items-center justify-between">
          <span className="text-xs text-muted-foreground">{workflow.steps[currentStepIndex].label}...</span>
          <Button variant="ghost" size="sm" className="h-6 text-xs gap-1" onClick={onSkip}>
