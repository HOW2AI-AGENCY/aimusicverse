  ...
  for (const line of lines) {
    ...
    if (currentSection) {
      ...
      // Check for recommended tags
        ...
        trimmedLine.toLowerCase().includes("рекомендуемые теги:") ||
...
export function StructureResultCard({ content, className }: StructureResultCardProps) {
  ...
  if (sections.length === 0) {
    return (
      <motion.div
        ...
        className={cn("mt-2 p-3 bg-background/80 rounded-xl border border-border/50", className)}
      >
        ...
        <div className="text-sm whitespace-pre-wrap leading-relaxed text-muted-foreground">{content}</div>
      </motion.div>
    );
  }
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: 5 }}
      animate={{ opacity: 1, y: 0 }}
      className={cn("mt-2 p-3 bg-background/80 rounded-xl border border-border/50 space-y-3", className)}
    >
      {/* Header */}
      ...
      <div className="space-y-2">
        {sections.map((section, idx) => (
          <motion.div
            ...
            className={cn("p-2 rounded-lg border", getSectionColor(section.tag))}
          >
            <div className="flex items-center gap-2">
