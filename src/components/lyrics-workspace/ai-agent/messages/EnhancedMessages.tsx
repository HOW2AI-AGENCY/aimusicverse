...
export function LyricsGeneratedMessage({
  ...
  return (
    ...
    >
      ...
      {/* Lyrics preview */}
      <div className="p-3">
        <pre className="text-sm whitespace-pre-wrap font-sans leading-relaxed">{displayLines.join("\n")}</pre>
      ...
      {showApplyButton && (
        <div className="flex gap-2 px-3 pb-3">
          ...
          {onInsert && (
            <Button variant="outline" size="sm" onClick={() => onInsert(lyrics)} className="px-3">
              Вставить
            </Button>
          )}
        </div>
...
export function AnalysisMessage({ stats, onFix }: AnalysisMessageProps) {
  ...
  const scoreColor =
    stats.qualityScore >= 80 ? "text-emerald-400" : stats.qualityScore >= 60 ? "text-amber-400" : "text-red-400";
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-purple-500/10 bg-purple-500/10">
        ...
        <div className="flex items-center gap-2">
          <span className={cn("text-lg font-bold", scoreColor)}>{stats.qualityScore}</span>
    ...
    >
      ...
      {(warnCount > 0 || failCount > 0) && onAutoFix && (
        <div className="px-3 pb-3">
          <Button onClick={onAutoFix} variant="outline" size="sm" className="w-full gap-2">
            <Sparkles className="w-3.5 h-3.5" />
