...
export function AIToolbar({ activeTool, onSelectTool, isLoading, className, compact = false }: AIToolbarProps) {
  ...
  return (
    <div className={cn("w-full", className)}>
      <div ref={scrollRef} className="overflow-x-auto scrollbar-hide">
        <div className={cn("flex gap-1 p-1.5 min-w-max items-center", compact && "gap-0.5 p-1")}>
  ...
  return (
    <motion.button
      ...
      className={cn(
        ...
        compact ? "px-2 py-1.5 min-w-[3rem]" : "px-2.5 py-1.5 min-w-[3.5rem]",
        isActive ? `${tool.bgColor} border-2` : "bg-muted/30 border-border/50 hover:bg-muted/50 active:scale-95",
    ...
    >
      ...
      >
        {isLoading ? (
          <Loader2 className={cn("animate-spin", compact ? "w-3 h-3" : "w-3.5 h-3.5", tool.color)} />
        ) : (
          <Icon
            className={cn(
              "transition-colors",
