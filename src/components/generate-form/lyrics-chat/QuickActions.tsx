...
export function QuickActions({ hasLyrics = false, onActionSelect, className }: QuickActionsProps) {
  ...
  return (
    <Card className={cn("overflow-hidden", className)}>
      <div className="p-3 border-b bg-muted/30">
        ...
        <p className="text-xs text-muted-foreground mt-1">Выберите готовый запрос или напишите свой</p>
      </div>
              ...
              {Object.entries(QUICK_ACTIONS_BY_CATEGORY).map(([category, actions]) => {
                ...
                return (
                  <div key={category}>
                    ...
                    <div className="space-y-2">
                      {actions.map((action) => (
                        <motion.div key={action.id} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}>
                          <Button
                            variant="ghost"
