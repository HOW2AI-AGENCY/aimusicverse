...
import { Coins, Shield, Zap, AlertTriangle, History, Settings2 } from 'lucide-react';
  ...
  return (
    <div className="flex items-center justify-between gap-2 w-full">
      ...
      <div className="flex items-center gap-1.5 flex-shrink min-w-0">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              {isAdmin ? (
                <Badge
                  variant="outline"
                  className="gap-0.5 px-1.5 py-0.5 cursor-help border-amber-500/50 bg-amber-500/10 text-amber-600 dark:text-amber-400 text-[10px]"
                  aria-label={`API баланс: ${userBalance} кредитов`}
                >
                  <Shield className="w-2.5 h-2.5" />
                  <Zap className="w-2.5 h-2.5" />
                  <span className="font-semibold">{Math.floor(userBalance)}</span>
                </Badge>
              ) : (
                <Badge
                  variant={canGenerate ? "secondary" : "destructive"}
                  className="gap-0.5 px-1.5 py-0.5 cursor-help text-[10px]"
                  aria-label={`Ваш баланс: ${userBalance} кредитов`}
                >
                  {!canGenerate && <AlertTriangle className="w-2.5 h-2.5" />}
        ...
        {/* Mode Toggle - icons on mobile, text on desktop */}
        <div className="flex items-center gap-0.5 p-0.5 rounded-md bg-muted">
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent side="bottom" className="text-xs">Simple</TooltipContent>
            </Tooltip>
          </TooltipProvider>
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent side="bottom" className="text-xs">Custom</TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </div>
      </div>

      {/* Right side: Model + History */}
        ...
        {onOpenHistory && (
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent side="bottom" className="text-xs">История</TooltipContent>
            </Tooltip>
          </TooltipProvider>
        )}
      </div>
