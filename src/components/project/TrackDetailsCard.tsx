...
export function TrackDetailsCard({ track, compact = false, className }: TrackDetailsCardProps) {
  const hasParams =
    track.bpm_target || track.key_signature || track.energy_level || track.vocal_style || track.duration_target;
  ...
  return (
    <div className={cn("rounded-lg bg-muted/30 border border-border/30", compact ? "p-2" : "p-2.5", className)}>
      {/* Main Parameters Row */}
      {hasParams && (
      ...
      {hasStylePrompt && !compact && (
        <div className="mb-2">
          ...
          <p className="text-xs text-foreground/80 line-clamp-2">{track.style_prompt}</p>
        </div>
      )}
