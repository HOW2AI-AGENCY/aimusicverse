...
export function UnlinkedTracksSection({ projectId, projectTracks }: UnlinkedTracksSectionProps) {
  ...
  const { unlinkedTracks, isLoading, linkTrackToSlot, isLinking } = useProjectGeneratedTracks(projectId);
    ...
    return (
      ...
      >
        ...
        <div
          ...
          onClick={() =>
            isTrackPlaying ? pauseTrack() : playTrack(track as unknown as Parameters<typeof playTrack>[0])
          }
        >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            <span className="text-sm font-medium truncate">{track.title || "Без названия"}</span>
          </div>
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            {track.style && <span className="truncate">{track.style.slice(0, 20)}</span>}
          </div>
        </div>
    ...
    <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-amber-700 dark:text-amber-400">Непривязанные треки</span>
          ...
          >
            <div className="p-3 space-y-2">
              <p className="text-xs text-muted-foreground mb-2">
                Эти треки были сгенерированы для проекта, но не привязаны к конкретным слотам. Выберите слот и нажмите
                "Привязать".
              </p>
