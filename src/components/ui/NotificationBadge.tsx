      ...
      <AnimatePresence>
        {showBadge && (
          ...
          >
            ...
            {pulse && (
              <span
                className={cn("absolute inset-0 rounded-full animate-ping", variantClasses[variant], "opacity-75")}
              />
            )}
...
export function StatusIndicator({ status, size = "md", pulse = true, className }: StatusIndicatorProps) {
  ...
  return (
    ...
    >
      {pulse && status === "online" && (
        <span className={cn("absolute inset-0 rounded-full animate-ping", statusColors[status], "opacity-75")} />
      )}
    </span>
  );
}
