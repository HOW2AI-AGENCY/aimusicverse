...
export function showToast(message: string, opts: ToastOptions = {}) {
  ...
  switch (type) {
    case 'success': return notify.success(message, rest);
    case 'error': return notify.error(message, rest);
    case 'warning': return notify.warning(message, rest);
    case 'loading': return notify.progress(message, rest);
    default: return notify.info(message, rest);
  }
}
