  ...
  if (variant === "icon") {
    return (
      <Button
        ...
        className={cn("h-9 w-9 rounded-full transition-all duration-300", "hover:bg-accent hover:scale-105", className)}
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="min-w-[140px]">
        <DropdownMenuItem onClick={() => handleSelect("light")} className={cn(theme === "light" && "bg-accent")}>
          <Sun className="mr-2 h-4 w-4" />
          Светлая
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => handleSelect("dark")} className={cn(theme === "dark" && "bg-accent")}>
          <Moon className="mr-2 h-4 w-4" />
          Тёмная
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => handleSelect("system")} className={cn(theme === "system" && "bg-accent")}>
