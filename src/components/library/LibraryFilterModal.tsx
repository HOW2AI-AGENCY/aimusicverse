 ...
 * Uses UnifiedDialog (Sheet on mobile, Modal on desktop)
 */
    ...
    >
      <div className="space-y-6 p-4 sm:p-0">
            ...
            {FILTERS.map((filter) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <Label htmlFor={filter.id} className="flex-1 cursor-pointer font-medium text-sm">
                    {filter.label}
                  </Label>
            ...
            {SORTS.map((sort) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    "flex items-center space-x-3 rounded-lg border-2 p-3 cursor-pointer transition-colors min-h-[56px]",
                    localSort === sort.id ? "border-primary bg-primary/10" : "border-border hover:border-primary/50",
                  )}
                ...
                >
                  ...
                  />
                  <Label htmlFor={sort.id} className="flex-1 cursor-pointer font-medium text-sm">
                    {sort.label}
                  </Label>
                </div>
              );
              ...
              {STATUS_OPTIONS.map((status) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <Label htmlFor={`status-${status.id}`} className="flex-1 cursor-pointer font-medium text-sm">
                      {status.label}
                    </Label>
                    {count !== undefined && count > 0 && (
        ...
        <div className="flex gap-2 pt-2">
          <Button type="button" variant="outline" onClick={handleReset} className="flex-1 min-h-[44px]">
            Сбросить
          </Button>
          <Button type="button" onClick={handleApply} className="flex-1 min-h-[48px]">
            Применить
          </Button>
        </div>
      </div>
