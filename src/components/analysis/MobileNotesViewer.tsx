...
export const MobileNotesViewer = memo(function MobileNotesViewer({
  ...
  if (notes.length === 0) {
    return (
      <div className={cn("rounded-lg bg-muted/30 flex flex-col items-center justify-center gap-3 p-6", className)}>
        ...
        <div className="overflow-x-auto overflow-y-hidden touch-pan-x" style={{ WebkitOverflowScrolling: "touch" }}>
          {viewMode === "piano" && (
            <div className="min-w-[100%] xs:min-w-[350px]">
              <InteractivePianoRoll notes={notes} duration={duration} height={220} onNoteClick={handleNoteClick} />
            </div>
          )}
              ...
              {processedNotes.slice(0, 150).map((note, i) => (
                ...
                >
                  ...
                  <div className="text-right flex flex-col items-end gap-1">
                    <span className="text-xs text-muted-foreground">{(note.duration * 1000).toFixed(0)} мс</span>
                    <div className="h-1.5 w-16 rounded-full bg-muted" title={`Velocity: ${note.velocity}`}>
                      <div
        ...
        {pdfUrl ? (
          ...
          onGeneratePdf && (
            <Button
              ...
              className="h-12 gap-2"
            >
              {isGeneratingPdf ? <Zap className="w-5 h-5 animate-pulse" /> : <FileText className="w-5 h-5" />}
