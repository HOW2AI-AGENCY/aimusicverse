    ...
    <div className="flex items-center gap-1">
      {shortcut.keys.map((key, idx) => (
        <span key={key} className="flex items-center gap-1">
          ...
          {idx < shortcut.keys.length - 1 && <span className="text-xs text-muted-foreground">+</span>}
        </span>
      ))}
    </div>
  </div>
...
export const KeyboardShortcutsDialog = memo(({ open, onOpenChange, trigger }: KeyboardShortcutsDialogProps) => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="max-w-md max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Быстрый доступ к функциям студии с клавиатуры</DialogDescription>
        </DialogHeader>
