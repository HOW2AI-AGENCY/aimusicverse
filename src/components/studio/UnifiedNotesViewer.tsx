...
export const UnifiedNotesViewer = memo(function UnifiedNotesViewer({
  ...
  const { parseMusicXmlFromUrl, parsedXml, isLoading: isParsingXml, error: musicXmlError } = useMusicXmlParser();
    ...
    <div className={cn("flex flex-col gap-2", className)}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[9px] text-muted-foreground">
                            октава {Math.floor(note.pitch / 12) - 1}
                          </span>
                        </div>
      ...
      {enablePlayback && viewMode !== "list" && notes.length > 0 && (
        <div className="flex items-center gap-2">
          ...
          >
            {internalPlaying ? <Pause className="w-3.5 h-3.5" /> : <Play className="w-3.5 h-3.5 ml-0.5" />}
          </Button>
          ...
          {!isMobile && (
            <div className="flex items-center gap-1">
              <Button size="icon" variant="ghost" className="h-6 w-6" onClick={() => setMuted(!isMuted)}>
