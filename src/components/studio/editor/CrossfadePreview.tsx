...
export function CrossfadePreview({
  ...
  const isCrossfading =
    currentTime >= crossfadePoint - crossfadeDuration / 2 && currentTime <= crossfadePoint + crossfadeDuration / 2;

  return (
    <div className={cn("rounded-xl border border-border/50 bg-card/80 backdrop-blur-sm overflow-hidden", className)}>
      {/* Visual indicator */}
      <div className="relative h-16 bg-gradient-to-r from-muted/30 to-muted/30 overflow-hidden">
        {/* Progress bar */}
        <motion.div className="absolute inset-y-0 left-0 bg-primary/20" style={{ width: `${progress}%` }} />
        ...
        {/* Playhead */}
        <motion.div className="absolute top-0 bottom-0 w-0.5 bg-primary shadow-lg" style={{ left: `${progress}%` }} />
      </div>
        ...
        <div className="flex items-center gap-1 bg-muted/30 rounded-lg p-0.5">
          {(["original", "crossfade", "replacement"] as PreviewMode[]).map((m) => (
            <Button
              ...
              size="sm"
              onClick={() => setMode(m)}
              className={cn("flex-1 h-7 text-xs", mode === m && "shadow-sm")}
            >
              {m === "original" && "Оригинал"}
              {m === "crossfade" && "↔ Переход"}
