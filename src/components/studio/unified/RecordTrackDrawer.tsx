...
export const RecordTrackDrawer = memo(function RecordTrackDrawer({
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent className="max-h-[90vh]" style={{ paddingBottom: safeAreaBottom }}>
        <div style={{ paddingTop: safeAreaTop }}>
          <DrawerHeader className="text-center pb-3">
            ...
            <p className="text-xs text-muted-foreground mt-1">Запишите аудио напрямую в студийный проект</p>
          </DrawerHeader>
          ...
          <div className="px-4 pb-6 space-y-5">
            ...
            {hasPermission === false && (
              <div className="flex items-center gap-3 p-4 rounded-xl bg-destructive/10 text-destructive">
                ...
                <div className="text-sm">
                  ...
                  <p className="text-destructive/70">Разрешите доступ в настройках браузера</p>
                </div>
              </div>
            )}
            ...
            {!isRecording && !isUploading && (
              <div className="space-y-3">
                ...
                {/* Type description */}
                <p className="text-xs text-muted-foreground text-center">{TYPE_CONFIG[recordingType].description}</p>
              </div>
            )}
            ...
            {isRecording && (
              ...
              >
                ...
                {recordingType === "guitar" && chordDetection.currentChord && (
                  ...
                  >
                    <span className="text-2xl font-bold text-primary">{chordDetection.currentChord.name}</span>
                    <div className="w-28 h-28 sm:w-32 sm:h-32">
                      <ChordDiagram chord={chordDetection.currentChord.name} size="sm" />
                    </div>
                  </motion.div>
                )}
