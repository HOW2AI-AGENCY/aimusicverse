...
export const UnifiedStudioMobile = memo(function UnifiedStudioMobile({
  ...
  if (!dawProject) {
    return (
      <div className={cn("flex h-screen w-full items-center justify-center p-4", className)} data-testid="studio-error">
      ...
      >
        <UnifiedDAWLayout
          ...
          onTrackMuteToggle={(trackId) => {
            patterns.tap();
            studio.toggleMute(trackId);
          }}
          onTrackSoloToggle={(trackId) => {
            patterns.tap();
            studio.toggleSolo(trackId);
          }}
