    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="h-[85vh] rounded-t-2xl p-0">
        <SheetHeader className="p-4 pb-2 border-b">
          ...
          {/* Mode Tabs */}
          <Tabs value={musicLab.mode} onValueChange={(v) => musicLab.setMode(v as MusicLabMode)} className="mt-2">
            <TabsList className="w-full grid grid-cols-3 h-10">
              {MODE_TABS.map((tab) => {
                const Icon = tab.icon;
                const isLocked = tab.value === "promptdj" && !isPro;
                return (
                  <TabsTrigger key={tab.value} value={tab.value} className="h-9 gap-1.5 relative" disabled={isLocked}>
                    <Icon className="h-4 w-4" />
                    <span className="text-xs">{tab.label}</span>
                    {isLocked && <Lock className="h-3 w-3 absolute -top-0.5 -right-0.5 text-muted-foreground" />}
                  </TabsTrigger>
                );
              })}
            </TabsList>
              ...
              {musicLab.mode === "record" && (
                ...
                >
                  ...
                  {!musicLab.isRecording && !musicLab.isUploading && (
                    <>
                      <p className="text-sm text-muted-foreground text-center">Выберите тип записи</p>
                  ...
                  {musicLab.isRecording && (
                    <div className="space-y-6">
                      ...
                      <div className="flex items-center justify-center gap-1 h-20">
                        {Array.from({ length: barCount }).map((_, i) => {
                          ...
                          return (
                            <motion.div
                              key={i}
                              className={cn("w-2 rounded-full transition-colors", isActive ? "bg-primary" : "bg-muted")}
                              animate={{
                                height: isActive ? 30 + Math.random() * 50 : 8,
                              }}
                      ...
                      {musicLab.recordingType === "guitar" && musicLab.chordDetection.currentChord && (
                        <Card>
                          <CardContent className="p-4 text-center">
                            <p className="text-3xl font-bold font-mono">{musicLab.chordDetection.currentChord.name}</p>
                      ...
                      <div className="flex gap-3">
                        <Button variant="outline" size="lg" className="flex-1 h-14" onClick={musicLab.cancelRecording}>
                          <X className="h-5 w-5 mr-2" />
                          Отмена
                        </Button>
                        <Button variant="destructive" size="lg" className="flex-1 h-14" onClick={handleRecordToggle}>
                      ...
                      <CardContent className="p-4">
                        ...
                        <div className="space-y-2">
                          {musicLab.sessionRecordings.map((track) => (
                            <div key={track.id} className="flex items-center gap-3 p-2 rounded-lg bg-muted/30">
                              <span
                                className={cn(
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {track.chords && track.chords.length > 0 && (
                                  <p className="text-xs text-muted-foreground">{track.chords.length} аккордов</p>
                                )}
                              </div>
                              <Button
                                variant="ghost"
              ...
              {musicLab.mode === "chords" && (
                ...
                >
                    ...
                    <CardContent className="p-6 text-center">
                      ...
                      {musicLab.chordDetection.currentChord && (
                        <p className="text-sm text-muted-foreground">
                          {musicLab.chordDetection.currentChord.quality} • Точность:{" "}
                          {Math.round(musicLab.chordDetection.currentChord.confidence * 100)}%
                        </p>
                      )}
                  ...
                  {musicLab.chordDetection.error && (
                    <p className="text-sm text-destructive text-center">{musicLab.chordDetection.error}</p>
                  )}
                </motion.div>
              )}
                              ...
                              {musicLab.promptDJ.generatedTracks.map((track) => (
                                <div key={track.id} className="flex items-center gap-3 p-2 rounded-lg bg-muted/30">
                                  <Button
                                    ...
                                    onClick={() => {
                                      if (
                                        musicLab.promptDJ.currentTrack?.id === track.id &&
                                        musicLab.promptDJ.isPlaying
                                      ) {
