...
export const StudioShell = memo(function StudioShell({ className }: StudioShellProps) {
  ...
  const handleBack = useCallback(() => {
    if (hasUnsavedChanges) {
      if (confirm("Есть несохранённые изменения. Выйти?")) navigate(-1);
    } else navigate(-1);
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.code) {
        ...
        case "KeyZ":
          ...
          break;
        case "KeyM":
          if (project?.tracks[0]) toggleTrackMute(project.tracks[0].id);
          break;
        case "KeyS":
          if (!e.ctrlKey && !e.metaKey && project?.tracks[0]) toggleTrackSolo(project.tracks[0].id);
          break;
        case "Digit1":
          if (project?.tracks[0]?.versions?.[0])
            setTrackActiveVersion(project.tracks[0].id, project.tracks[0].versions[0].label);
          break;
        case "Digit2":
          if (project?.tracks[0]?.versions?.[1])
            setTrackActiveVersion(project.tracks[0].id, project.tracks[0].versions[1].label);
          break;
        case "ArrowLeft":
          e.preventDefault();
  ...
  useEffect(() => {
    ...
    if (importedStemsForTrackRef.current !== srcId) {
      ...
      (async () => {
        try {
          ...
          if (error) {
            logger.warn("Failed to load track stems", { error });
            return;
          }
    ...
    const channel = supabase
      .channel(`studio-stems-${srcId}`)
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "track_stems", filter: `track_id=eq.${srcId}` },
        (payload) => {
          if (!isMountedRef.current) return;
  ...
  return (
    ...
    >
      ...
      <StudioShellDialogs
        ...
        showPresetsSheet={showPresetsSheet}
        setShowPresetsSheet={setShowPresetsSheet}
        showDashboardSheet={showDashboardSheet}
        setShowDashboardSheet={setShowDashboardSheet}
        showMixerSheet={showMixerSheet}
        setShowMixerSheet={setShowMixerSheet}
        showMusicLabSheet={showMusicLabSheet}
        setShowMusicLabSheet={setShowMusicLabSheet}
        ...
        setShowEffectsDrawer={setShowEffectsDrawer}
        showAddVocalsDrawer={showAddVocalsDrawer}
        setShowAddVocalsDrawer={setShowAddVocalsDrawer}
        ...
        showGenerateSheet={showGenerateSheet}
        setShowGenerateSheet={setShowGenerateSheet}
        ...
        showNotationPanel={showNotationPanel}
        setShowNotationPanel={setShowNotationPanel}
        showTranscriptionPanel={showTranscriptionPanel}
        setShowTranscriptionPanel={setShowTranscriptionPanel}
        showArrangementDialog={showArrangementDialog}
        setShowArrangementDialog={setShowArrangementDialog}
        showInstrumentalResult={showInstrumentalResult}
        setShowInstrumentalResult={setShowInstrumentalResult}
        showStemSeparationDialog={showStemSeparationDialog}
        setShowStemSeparationDialog={setShowStemSeparationDialog}
        selectedEffectsTrack={selectedEffectsTrack}
        setSelectedEffectsTrack={setSelectedEffectsTrack}
        selectedVocalsTrack={selectedVocalsTrack}
        setSelectedVocalsTrack={setSelectedVocalsTrack}
        selectedExtendTrack={selectedExtendTrack}
        setSelectedExtendTrack={setSelectedExtendTrack}
        selectedSectionTrack={selectedSectionTrack}
        setSelectedSectionTrack={setSelectedSectionTrack}
        selectedNotationTrack={selectedNotationTrack}
        setSelectedNotationTrack={setSelectedNotationTrack}
        selectedTranscriptionTrack={selectedTranscriptionTrack}
        setSelectedTranscriptionTrack={setSelectedTranscriptionTrack}
        selectedArrangementTrack={selectedArrangementTrack}
        setSelectedArrangementTrack={setSelectedArrangementTrack}
        instrumentalResultData={instrumentalResultData}
        setInstrumentalResultData={setInstrumentalResultData}
        trackEffects={trackEffects}
        setTrackEffects={setTrackEffects}
        onSave={handleSave}
        onExport={handleExport}
        onBack={handleBack}
