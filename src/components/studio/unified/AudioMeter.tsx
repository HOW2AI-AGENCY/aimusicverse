...
export const AudioMeter = memo(function AudioMeter({
  ...
  if (isVertical) {
    return (
      <div className={cn("relative rounded-md bg-muted/50 overflow-hidden", className)} style={{ width, height }}>
        {/* Level bar */}
        <motion.div
          className={cn("absolute bottom-0 left-0 right-0 rounded-md", "bg-gradient-to-t", getGradient())}
        ...
        <div className="absolute inset-0 flex flex-col justify-between py-1 pointer-events-none">
          {[0, 25, 50, 75, 100].map((mark) => (
            <div key={mark} className="w-full h-px bg-background/30" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div
      className={cn("relative rounded-md bg-muted/50 overflow-hidden", className)}
      style={{ width: height, height: width }}
    >
      {/* Level bar */}
      <motion.div
        className={cn("absolute top-0 bottom-0 left-0 rounded-md", "bg-gradient-to-r", getGradient())}
...
export const StereoMeter = memo(function StereoMeter({
  ...
  return (
    <div className={cn("flex gap-1", className)}>
      <AudioMeter analyserNode={leftAnalyser} isPlaying={isPlaying} height={height} width={6} orientation="vertical" />
      <AudioMeter analyserNode={rightAnalyser} isPlaying={isPlaying} height={height} width={6} orientation="vertical" />
    </div>
  );
});
  ...
  return (
    ...
    >
      <motion.div
        ...
        animate={isVertical ? { height: `${animatedLevel * 100}%` } : { width: `${animatedLevel * 100}%` }}
