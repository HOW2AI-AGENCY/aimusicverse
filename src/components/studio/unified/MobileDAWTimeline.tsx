...
const getTrackConfig = (type: string) => TRACK_TYPE_CONFIG[type.toLowerCase()] || TRACK_TYPE_CONFIG.other;
  ...
  return (
    <div className={cn("flex flex-col bg-card rounded-lg overflow-hidden", className)}>
      ...
      <div className="h-8 bg-muted/50 relative border-b border-border">
        {/* Progress indicator */}
        <div className="absolute top-0 left-0 h-full bg-primary/30" style={{ width: `${progressPercent}%` }} />
        ...
        {/* Playhead indicator */}
        <div className="absolute top-0 h-full w-0.5 bg-primary" style={{ left: `${progressPercent}%` }} />
      </div>

      {/* Timeline area */}
      <div ref={containerRef} className="flex-1 overflow-x-auto overflow-y-hidden touch-pan-x" {...bind()}>
        <div
          ref={timelineRef}
        ...
        >
          ...
          >
            ...
            <div className={cn("absolute left-1/2 -translate-x-1/2 bottom-0 w-3 h-3", "bg-primary/80 rounded-sm")} />
          </div>
        </div>
      </div>
    </div>
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          <span className="text-sm font-medium truncate text-foreground">{track.name}</span>
        </div>
      ...
      <div className="h-10 bg-background/30 rounded overflow-hidden">
        {track.audioUrl ? (
          ...
          <div className="h-full flex items-center gap-px px-1" style={{ opacity: effectiveMuted ? 0.3 : 1 }}>
            {Array.from({ length: 30 }).map((_, i) => {
              const height = 20 + Math.sin(i * 0.5) * 15 + Math.random() * 10;
              return <div key={i} className="flex-1 bg-foreground/20 rounded-sm" style={{ height: `${height}%` }} />;
            })}
          </div>
        )}
      </div>
