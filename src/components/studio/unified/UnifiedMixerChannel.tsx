  ...
  if (variant === "vertical") {
    return (
      ...
      >
        ...
        {/* Name */}
        <p className="text-xs font-medium text-center truncate w-full mb-2">{name || config.shortLabel}</p>

        {/* Level Meter + Fader */}
        <div className="flex items-end gap-1 h-32 mb-3">
          {showMeter && <SimpleMeter volume={volume} isMuted={muted} isPlaying={isPlaying} height={120} width={6} />}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        {/* Icon */}
        <div className={cn("flex items-center justify-center w-10 h-10 rounded-lg flex-shrink-0", config.color)}>
          <Icon className="w-5 h-5" />
        </div>
