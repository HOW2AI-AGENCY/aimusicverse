...
export const StudioWaveformTimeline = memo(function StudioWaveformTimeline({
  ...
  return (
    ...
    >
      ...
      {isReady && (
        <div
          className="absolute top-0 bottom-0 w-0.5 bg-primary z-20 pointer-events-none shadow-lg"
          style={{ left: `${progress}%` }}
        >
          <div className="absolute -top-0.5 left-1/2 -translate-x-1/2 w-2.5 h-2.5 bg-primary rounded-full shadow" />
        </div>
      )}
      ...
      {isReady && (
        <div 
          className="absolute bottom-1 px-1.5 py-0.5 rounded bg-primary/90 text-primary-foreground text-[10px] font-mono pointer-events-none z-30"
          style={{ 
