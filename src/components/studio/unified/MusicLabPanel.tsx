        ...
        <Tabs value={activeMode} onValueChange={(v) => setActiveMode(v as LabMode)}>
          <TabsList className="w-full grid grid-cols-3 h-10">
            ...
            <TabsTrigger value="promptdj" className="h-9 gap-1.5 relative">
              ...
              {!isPro && <Lock className="h-3 w-3 absolute -top-0.5 -right-0.5 text-muted-foreground" />}
            </TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {activeMode === "record" && (
            <motion.div
              ...
              className="space-y-3"
            >
              <p className="text-sm text-muted-foreground text-center mb-4">Выберите тип записи</p>
                ...
                return (
                  ...
                  >
                    <CardContent className="p-4 flex items-center gap-4">
                      <span
                        className={cn("w-12 h-12 rounded-xl flex items-center justify-center shrink-0", option.color)}
                      >
          ...
          {activeMode === "chords" && (
            ...
            >
                ...
                <CardContent className="p-6 text-center">
                  <AnimatePresence mode="wait">
                    <motion.div
                      ...
                      className="mb-4"
                    >
                      <span className="text-5xl font-bold font-mono">{chordDetection.currentChord?.name || "—"}</span>
                    </motion.div>
                  </AnimatePresence>

                  {chordDetection.currentChord && (
                    <p className="text-sm text-muted-foreground">
                      {chordDetection.currentChord.quality} • Confidence:{" "}
                      {Math.round(chordDetection.currentChord.confidence * 100)}%
                    </p>
                  )}
                    ...
                    <div className="flex gap-0.5 h-8">
                      {Array.from({ length: 16 }).map((_, i) => {
                        ...
                        return (
                          <motion.div
                            key={i}
                            className={cn("w-1.5 rounded-full transition-colors", isActive ? "bg-primary" : "bg-muted")}
                            animate={{
                ...
                <Card>
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Button variant="ghost" size="sm" onClick={chordDetection.clearHistory} className="h-7">
              ...
              {chordDetection.error && <p className="text-sm text-destructive text-center">{chordDetection.error}</p>}
            </motion.div>
          )}
                ...
                <Card className="overflow-hidden">
                  <CardContent className="p-6 text-center">
                    ...
                    <p className="text-sm text-muted-foreground mb-4">
                      Создавайте музыку с помощью AI DJ интерфейса. Настраивайте каналы, миксуйте стили и генерируйте
                      уникальные треки.
                    </p>
                      ...
                      {promptDJ.channels
                        ...
                        .map((channel) => (
                          <div key={channel.id} className="flex items-center gap-3">
                            ...
                            <span className="text-xs text-muted-foreground">x{channel.weight.toFixed(1)}</span>
                          </div>
                        ))}
                    </CardContent>
                  </Card>
                          ...
                          {promptDJ.generatedTracks.map((track) => (
                            <div key={track.id} className="flex items-center gap-3 p-2 rounded-lg bg-muted/30">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-muted-foreground">{track.createdAt.toLocaleTimeString()}</p>
                              </div>
                            </div>
                          ))}
                        </div>
