...
export function TimelineRuler({ duration, zoom, bpm, className }: TimelineRulerProps) {
  ...
  return (
    <div
      className={cn("h-8 bg-muted/50 border-b border-border/30 relative sticky top-0 z-10", className)}
      style={{ width: duration * zoom + 200 }}
    >
      {/* BPM indicator */}
      <div className="absolute left-2 top-1/2 -translate-y-1/2 text-xs text-muted-foreground">{bpm} BPM</div>
