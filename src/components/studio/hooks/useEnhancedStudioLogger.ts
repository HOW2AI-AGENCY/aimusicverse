...
export type StudioState = "idle" | "selecting" | "editing" | "processing" | "comparing" | "mixing";
...
export function useEnhancedStudioLogger(trackId: string) {
  ...
  const logSectionPreview = useCallback(
    (start: number, end: number) => logActivity("section_preview", { sectionStart: start, sectionEnd: end }),
    [logActivity],
  );
  ...
  const logReplacementComplete = useCallback(
    (versionId: string) => {
  ...
  const logStemSeparateComplete = useCallback(
    (stemCount: number) => {
      updateState("idle");
  ...
  const logTipView = useCallback((tipId: string) => logActivity("tip_view", { tipId }), [logActivity]);

  const logTipDismiss = useCallback((tipId: string) => logActivity("tip_dismiss", { tipId }), [logActivity]);

  const logTipAction = useCallback(
    (tipId: string, actionId: string) => logActivity("tip_action", { tipId, value: actionId }),
    [logActivity],
  );

  const logOnboardingStart = useCallback(() => logActivity("onboarding_start"), [logActivity]);

  const logOnboardingComplete = useCallback(() => logActivity("onboarding_complete"), [logActivity]);

  const logOnboardingSkip = useCallback(
    (step: number) => logActivity("onboarding_skip", { value: step }),
    [logActivity],
  );
