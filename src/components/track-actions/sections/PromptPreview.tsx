...
export const PromptPreview = memo(function PromptPreview({
  ...
  const truncatedText = isLong && !expanded ? lines.slice(0, maxLines).join("\n").slice(0, 150) + "..." : displayText;
    ...
    <div className={cn("rounded-lg bg-muted/30 border border-border/50 overflow-hidden", className)}>
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border/30">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-muted-foreground">{prompt ? "Промпт" : "Стиль"}</span>
        </div>
      ...
      {/* Content */}
      <div className="px-3 py-2 cursor-pointer" onClick={isLong ? toggleExpand : undefined}>
        <p
          className={cn(
