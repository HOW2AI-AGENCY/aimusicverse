...
export function WatermarkActions({ track, onAction }: WatermarkActionsProps) {
  ...
  return (
    <>
      ...
      >
        {isApplying ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Shield className="h-4 w-4 mr-2" />}
        {track.is_watermarked === true ? "Водяной знак добавлен" : "Добавить водяной знак"}
      </DropdownMenuItem>

      <DropdownMenuItem onClick={handleDetectWatermark} disabled={isLoading || !track.audio_url}>
        {isDetecting ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <ShieldCheck className="h-4 w-4 mr-2" />}
