...
export function MidiTranscriptionActions({
    ...
    if (!hasAccess) {
      return (
        <DropdownMenuItem disabled className="text-muted-foreground">
          ...
          MIDI транскрипция
          <span className="ml-auto text-xs opacity-60">{requiredTier?.toUpperCase()}</span>
        </DropdownMenuItem>
      );
    }
          ...
          {stems.map((stem) => {
            ...
            return (
              ...
              >
                ...
                {label} → MIDI
                <span className="ml-auto text-xs text-muted-foreground">{config.model}</span>
              </DropdownMenuItem>
            );
          })}
          <DropdownMenuSeparator />
          <DropdownMenuItem onClick={() => onAction("transcribe_notes")} disabled={isProcessing}>
            <Music2 className="w-4 h-4 mr-2" />
            Все стемы → Ноты
          </DropdownMenuItem>
        </DropdownMenuSubContent>
  ...
  if (!hasAccess) {
    return (
      <IconGridButton icon={Lock} label="MIDI" color="muted" onClick={() => onAction("transcribe_midi")} disabled />
    );
  }
