...
export const LyricsPreview = memo(function LyricsPreview({ lyrics, maxSections = 4, className }: LyricsPreviewProps) {
  ...
  return (
    <>
      <div className={cn("rounded-lg bg-muted/30 border border-border/50 overflow-hidden", className)}>
        {/* Header */}
        <div className="flex items-center justify-between px-3 py-2 border-b border-border/30">
          <div className="flex items-center gap-2">
            <Music className="w-3.5 h-3.5 text-muted-foreground" />
            <span className="text-xs font-medium text-muted-foreground">Текст ({parsed.sections.length} секций)</span>
          </div>
        ...
        {/* Section badges */}
        <div className="px-3 py-2 cursor-pointer" onClick={openFullLyrics}>
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent side="bottom" className="h-[70vh] max-h-[70vh] rounded-t-2xl flex flex-col pb-0 px-0">
            ...
            <div className="px-4 py-4 pb-safe space-y-4">
              {parsed.sections.map((section, index) => (
                <div key={section.id || index} className="space-y-1.5">
                  <Badge
                    variant="secondary"
                    className={cn("text-xs px-2 py-0.5 font-medium", SECTION_COLORS[section.type])}
                  >
                    {section.name}
                  </Badge>
                  <p className="text-sm text-foreground/90 whitespace-pre-wrap pl-0.5">{section.content}</p>
                </div>
              ))}
            </div>
          </ScrollArea>
