...
export function CreateActions({ track, state, onAction, variant, isProcessing }: CreateActionsProps) {
  ...
  return (
    <div className="grid grid-cols-4 gap-1">
      ...
      {showCover && <IconGridButton icon={Disc} label="Кавер" color="purple" onClick={() => onAction("cover")} />}
      {showExtend && <IconGridButton icon={Plus} label="Расширить" color="green" onClick={() => onAction("extend")} />}
      {showRemix && (
        <IconGridButton
          icon={Music}
          label="Ремикс"
