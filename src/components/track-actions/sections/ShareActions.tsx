...
export function ShareActions({ track, state, onAction, variant, isProcessing }: ShareActionsProps) {
        ...
        <DropdownMenuSubContent className="bg-background/95 backdrop-blur-sm z-[10000]" sideOffset={8}>
          ...
          {showPlaylist && (
            <DropdownMenuItem onClick={() => onAction("add_to_playlist")}>
              <ListMusic className="w-4 h-4 mr-2" />В плейлист
            </DropdownMenuItem>
          )}
          {showProject && (
            <DropdownMenuItem onClick={() => onAction("add_to_project")}>
              <Folder className="w-4 h-4 mr-2" />В проект
            </DropdownMenuItem>
          )}
        </DropdownMenuSubContent>
      </DropdownMenuSub>
  ...
  return (
    <div className="grid grid-cols-4 gap-1 mt-1">
      ...
      {showCopyLink && (
        <IconGridButton icon={Link} label="Ссылка" color="muted" onClick={() => onAction("copy_link")} />
      )}
      {showPlaylist && (
        <IconGridButton icon={ListMusic} label="Плейлист" color="amber" onClick={() => onAction("add_to_playlist")} />
      )}
      {showProject && (
        <IconGridButton icon={Folder} label="Проект" color="green" onClick={() => onAction("add_to_project")} />
      )}
    </div>
  );
}
