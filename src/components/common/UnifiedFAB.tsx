      ...
      <AnimatePresence>
        {visible && (
          <motion.div
            ...
            className={cn("fixed left-0 right-0 z-50 px-4", className)}
            style={{ bottom: bottomStyle }}
          >
            <Button
              onClick={handleClick}
              disabled={disabled || loading}
              className={cn("w-full gap-2", touchTarget.large)}
            >
      ...
      {visible && (
        ...
        >
          ...
          <Button
            ...
            onClick={handleClick}
            disabled={disabled || loading}
            className={cn("rounded-full shadow-xl", touchTarget.icon, "w-14 h-14", isExpanded && "rotate-45")}
            style={{ transition: "transform 0.2s ease" }}
          >
            {isExpanded ? <X className="w-6 h-6" /> : Icon ? <Icon className="w-6 h-6" /> : null}
          </Button>
        </motion.div>
      )}
    </AnimatePresence>
