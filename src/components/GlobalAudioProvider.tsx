...
const AUDIO_ERROR_MESSAGES: Record<
  number,
  {
    ru: string;
    action?: string;
...
export function GlobalAudioProvider({ children }: { children: React.ReactNode }) {
  ...
  // Sync granular playback status from <audio> events → store, so UI can
  // render distinct loading / buffering / error states (not just play/pause).
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleError = () => {
      ...
      if (errorCode === 4 && !hasAttemptedBlobRecovery && formatRetryCount < MAX_FORMAT_RETRIES) {
        ...
        const allAudioUrls = [activeTrack?.streaming_url, activeTrack?.audio_url, activeTrack?.local_audio_url].filter(
          (url) => url && !attemptedUrls.has(url),
        ); // Remove null/undefined and already tried URLs
