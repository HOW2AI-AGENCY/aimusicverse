...
export const KnobCell = memo(
  function KnobCell({
    ...
    const handleTypeChangeClick = useCallback(
      (newType: ChannelType) => {
    ...
    return (
      <div className="flex flex-col items-center">
        <DropdownMenu open={isSelected} onOpenChange={handleOpenChange}>
          ...
          <DropdownMenuContent align="center" className="max-h-72 overflow-y-auto bg-popover z-50">
            {/* Change type submenu */}
            <DropdownMenuSub>
              <DropdownMenuSubTrigger className="text-xs">🎛️ Тип: {config.label}</DropdownMenuSubTrigger>
              <DropdownMenuSubContent className="max-h-60 overflow-y-auto bg-popover z-50">
                {CHANNEL_TYPES.map((typeConfig) => (
                  <DropdownMenuItem
                    ...
                    className={cn("text-xs", channel.type === typeConfig.type && "bg-accent")}
                  >
                    <div className="w-2 h-2 rounded-full mr-2" style={{ backgroundColor: typeConfig.color }} />
                    {typeConfig.label}
                  </DropdownMenuItem>
                ))}
              </DropdownMenuSubContent>
            ...
            {config.presets.length > 0 ? (
              config.presets.map((preset) => (
                <DropdownMenuItem key={preset} onClick={() => handlePresetSelect(preset)} className="text-xs">
                  {preset}
                </DropdownMenuItem>
              ))
            ) : (
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={handleToggleEnabled} className="text-xs">
