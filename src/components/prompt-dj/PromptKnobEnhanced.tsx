...
export const PromptKnobEnhanced = memo(function PromptKnobEnhanced({
  ...
  useEffect(() => {
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      const currentAngle =
        Math.atan2(touch.clientY - centerRef.current.y, touch.clientX - centerRef.current.x) * (180 / Math.PI);
  ...
  return (
    <div className="flex flex-col items-center gap-1.5 select-none touch-none">
      ...
      <motion.div
        ref={knobRef}
        className={cn("relative cursor-pointer rounded-full", !enabled && "opacity-40 cursor-not-allowed")}
        style={{
      ...
      >
        ...
        >
          ...
          {/* Value display */}
          <span className={cn("font-bold tabular-nums mt-2", sizeConfig.fontSize)} style={{ color: activeColor }}>
