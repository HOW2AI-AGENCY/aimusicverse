    ...
    <Card className="bg-card/50 backdrop-blur">
      <CardContent className="p-4 space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Slider value={[settings.bpm]} onValueChange={([bpm]) => onUpdate({ bpm })} min={60} max={180} step={1} />
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            ...
            <Select value={settings.key} onValueChange={(key) => onUpdate({ key })}>
          ...
          <div className="space-y-1.5">
            ...
            <Select value={settings.scale} onValueChange={(scale) => onUpdate({ scale })}>
              <SelectTrigger className="h-9">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground">
              {settings.density < 0.3 ? "Разреженно" : settings.density > 0.7 ? "Насыщенно" : "Средне"}
            </span>
          </div>
          <Slider
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground">
              {settings.brightness < 0.3 ? "Тёплый" : settings.brightness > 0.7 ? "Яркий" : "Нейтральный"}
            </span>
          </div>
          <Slider
        ...
        <div className="space-y-1.5">
          ...
          >
            {DURATION_OPTIONS.map((opt) => (
              <ToggleGroupItem key={opt.value} value={String(opt.value)} size="sm" className="h-8 px-3">
                {opt.label}
              </ToggleGroupItem>
            ))}
          </ToggleGroup>
