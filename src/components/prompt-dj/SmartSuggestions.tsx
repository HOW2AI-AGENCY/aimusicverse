...
const SUGGESTIONS_RULES: Array<{
  ...
  // Add bass for depth
  {
    condition: (channels, settings) => {
      const hasBass = channels.some((c) => c.enabled && c.value?.toLowerCase().includes("bass"));
      const hasHighEnergy = channels.some(
        (c) => c.type === "energy" && c.enabled && (c.value?.toLowerCase().includes("high") || c.weight > 0.7),
      );
  ...
  {
    condition: (channels, settings) => {
      const hasEDM = channels.some(
        (c) => c.enabled && ["edm", "house", "techno", "dance"].some((g) => c.value?.toLowerCase().includes(g)),
      );
  ...
  {
    condition: (channels, settings) => {
      const hasLoFi = channels.some((c) => c.enabled && c.value?.toLowerCase().includes("lo-fi"));
  ...
  {
    condition: (channels, settings) => {
      const hasCinematic = channels.some(
        (c) => c.enabled && ["cinematic", "epic", "orchestral", "film"].some((g) => c.value?.toLowerCase().includes(g)),
      );
      const hasStrings = channels.some((c) => c.enabled && c.value?.toLowerCase().includes("strings"));
    ...
    condition: (channels, settings) => {
      const hasHighEnergy = channels.some(
        (c) =>
          c.type === "energy" &&
          c.enabled &&
          (c.value?.toLowerCase().includes("high") || c.value?.toLowerCase().includes("intense") || c.weight > 0.8),
      );
  ...
  {
    condition: (channels, settings) => {
      const hasDarkMood = channels.some(
        (c) =>
          c.type === "mood" &&
          c.enabled &&
          ["dark", "sad", "melancholic", "mysterious"].some((m) => c.value?.toLowerCase().includes(m)),
      );
      ...
      <AnimatePresence mode="popLayout">
        {suggestions.map((suggestion) => (
          ...
          >
            ...
            <span className="flex-1 text-muted-foreground">{suggestion.text}</span>
