    ...
    <div className={cn("relative overflow-hidden rounded-lg", config.container, className)}>
      ...
      {onPlay && (
        ...
        >
          ...
          >
            {isPlaying ? <Pause className={config.playIcon} /> : <Play className={cn(config.playIcon, "ml-0.5")} />}
          </Button>
        </motion.div>
      )}
      ...
      {showDuration && duration && (
        <span className={cn("absolute bottom-1 right-1 text-white/80 rounded-full", pill.glassDark, config.duration)}>
