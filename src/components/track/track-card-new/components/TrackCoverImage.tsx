...
export const TrackCoverImage = memo(function TrackCoverImage({
  ...
  return (
    <div className={cn("relative flex-shrink-0 rounded-lg overflow-hidden shadow-md", sizeClasses[size], className)}>
      <LazyImage
        src={coverUrl || ""}
      ...
      <motion.div
        className={cn(
          ...
          isGridSize ? surface.imageGradient : surface.imageDark,
        ...
        <AnimatePresence mode="wait">
          {isPlaying ? (
            <motion.div key="playing" initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}>
              {isGridSize ? (
                <Button size="icon" className={cn(buttonSizeClasses[size], "rounded-full bg-primary")} onClick={onPlay}>
      ...
      {isPlaying && showPlayingGlow && (
        <motion.div
          ...
          animate={{
            boxShadow: ["0 0 0 0 hsl(var(--primary) / 0.4)", "0 0 0 4px hsl(var(--primary) / 0)"],
          }}
