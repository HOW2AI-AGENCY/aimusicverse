      ...
      >
        ...
        <div className="flex-1 min-w-0 space-y-1">
          <div className="flex items-center gap-2">
            <h3 className="font-semibold text-sm truncate leading-tight">{track.title || "Без названия"}</h3>

            {/* Version Switcher */}
            {versionCount > 1 && <VersionPills count={versionCount} onSwitch={(i) => onVersionSwitch?.(String(i))} />}
          </div>

          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground truncate max-w-[150px]">{track.style || "Нет стиля"}</span>

            {track.duration_seconds && (
              <span className="text-xs text-muted-foreground/70 font-mono">{formatTime(track.duration_seconds)}</span>
            )}
          </div>
        </div>

        {/* Status Icons */}
        <StatusIcons track={track} stemCount={stemCount} midiStatus={midiStatus} />
