...
export const MobileTextarea = memo(
  forwardRef<HTMLTextAreaElement, MobileTextareaProps>(function MobileTextarea(
    {
      value,
      onChange,
      placeholder,
        ...
        {showCounter && (maxLength || charCount > 0) && (
          <div
            className={cn(
              ...
              remaining !== null && remaining < 20 ? "text-destructive font-medium" : "text-muted-foreground",
            )}
          >
            {maxLength ? <span>{remaining} осталось</span> : <span>{charCount} символов</span>}
          </div>
        )}
      </div>
    );
