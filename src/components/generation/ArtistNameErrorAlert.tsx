...
export const ArtistNameErrorAlert = memo(function ArtistNameErrorAlert({
  ...
  return (
    <Card className={cn("border-amber-500/50 bg-amber-500/5 dark:bg-amber-950/20", "shadow-lg", className)}>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <div>
                <h4 className="font-semibold text-sm text-foreground">Имя артиста не поддерживается</h4>
                {artistName && (
            ...
            <p className="text-xs text-muted-foreground mt-2 leading-relaxed">
              Suno AI не поддерживает имена конкретных артистов. Опишите желаемый стиль музыки своими словами.
            </p>
                ...
                <div className="space-y-1.5">
                  {examples.slice(0, 2).map((example, index) => (
                    ...
                    >
                      ...
                      <span className="text-foreground font-medium truncate">{example.after}</span>
                    </div>
                  ))}
                </div>
              </div>
