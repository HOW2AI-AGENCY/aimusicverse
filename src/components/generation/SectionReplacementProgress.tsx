    ...
    <AnimatePresence>
      ...
      >
        <div className="space-y-3">
          ...
          {isActive && <Progress value={progress} className="h-2" />}
              ...
              <div className="grid grid-cols-2 gap-2">
                {variants.map((variant, idx) => (
                  ...
                  >
                    ...
                    >
                      {playingIndex === idx ? <Pause className="w-3 h-3" /> : <Play className="w-3 h-3" />}
                    </span>
                    <span className="ml-4">Вариант {variant.label}</span>
                    {selectedIndex === idx && <Check className="w-3 h-3 ml-auto" />}
                  </Button>
                ))}
              </div>
            </div>
