...
export function VersionTimeline({ trackId, activeVersionId, onVersionChange, className }: VersionTimelineProps) {
  ...
  const activeVersion =
    versions?.find((v) => v.is_primary) ?? (versions && versions.length > 0 ? versions[0] : undefined);
