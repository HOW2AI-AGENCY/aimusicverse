/**
 * Mobile Track Studio Layout
 * 
 * Tab-based interface for tracks without stems
 */
...
interface TrackStudioMobileLayoutProps {
  trackTitle: string;
  isPlaying: boolean;
  currentTime: number;
  duration: number;
...
const tabs: { id: MobileTab; label: string; icon: React.ElementType }[] = [
  ...
  { id: 'sections', label: 'Секции', icon: Scissors },
  { id: 'actions', label: 'Действия', icon: Wand2 },
];

export function TrackStudioMobileLayout({
  trackTitle,
  isPlaying,
  currentTime,
  duration,
  ...
  const availableTabs = tabs.filter(tab => 
    tab.id !== 'sections' || canReplaceSection
  );
  ...
  return (
    <div className="h-screen flex flex-col bg-background">
      ...
      {/* Tab Bar */}
      <nav className="flex items-center border-t border-border/30 bg-card/80 backdrop-blur-sm safe-area-bottom">
