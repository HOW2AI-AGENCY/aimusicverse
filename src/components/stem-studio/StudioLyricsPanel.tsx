...
export function StudioLyricsPanel({
  ...
  useEffect(() => {
    ...
    const activeIdx = lines.findIndex(
      (line) =>
        currentTime >= line.startTime - LINE_START_TOLERANCE && currentTime <= line.endTime + LINE_END_TOLERANCE,
    );
  ...
  const handleSmartSelectSection = useCallback(
    (lineIndex: number) => {
      if (!lines.length) return;
  ...
  const handleLineClick = useCallback(
    (line: LyricLine, globalIndex: number) => {
      if (selectionMode) {
        const now = Date.now();
        const lastClick = lastClickRef.current;

        // Check for double-click on same line
        if (lastClick && lastClick.index === globalIndex && now - lastClick.time < DOUBLE_CLICK_THRESHOLD) {
      ...
      <div className="px-4 sm:px-6 py-3 border-b border-border/30">
        ...
        <AnimatePresence>
          {!isCollapsed && (
            ...
            >
              <p className="text-sm text-muted-foreground whitespace-pre-wrap line-clamp-4">{plainLyrics}</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
    ...
    >
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span className="font-medium">{selectionMode ? "Выберите строки" : "Текст песни"}</span>
        ...
        <div className="flex items-center gap-2">
          {selectionMode && selectedLines.size > 0 && (
            <>
              <Button size="sm" variant="outline" onClick={() => setSelectedLines(new Set())} className="h-7 px-2">
                Очистить
              </Button>
              <Button
                size="sm"
          ...
          {selectionMode && !selectedLines.size && (
            <span className="text-xs text-muted-foreground hidden sm:block">Дважды нажмите для авто-выбора секции</span>
          )}
          <Button variant="ghost" size="sm" onClick={() => setIsCollapsed(!isCollapsed)} className="h-7 px-2">
            {isCollapsed ? <ChevronDown className="w-4 h-4" /> : <ChevronUp className="w-4 h-4" />}
          </Button>
