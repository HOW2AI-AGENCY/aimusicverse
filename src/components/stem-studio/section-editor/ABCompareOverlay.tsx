...
import { 
  ...
  ArrowLeftRight, Check, X, Sparkles, Wand2 
} from 'lucide-react';
...
export const ABCompareOverlay = memo(function ABCompareOverlay({
  ...
  const updateProgress = useCallback(() => {
    if (activeAudio) {
      const progress = activeVersion === 'original' 
            ...
            <div className="flex-1 h-10 flex items-center gap-0.5 overflow-hidden">
              {originalBars.map((height, i) => (
                <motion.div
                  ...
                  style={{ height: `${height}%` }}
                  animate={activeVersion === 'original' && isPlaying ? {
                    scaleY: [1, 1.1, 1]
                  } : {}}
            ...
            <div className="flex-1 h-10 flex items-center gap-0.5 overflow-hidden">
              {newBars.map((height, i) => (
                <motion.div
                  ...
                  className={cn(
                    'w-1 rounded-full transition-colors',
                    activeVersion === 'new' && (i / newBars.length) * 100 <= progress
                      ? 'bg-primary'
                      : 'bg-primary/30'
                  )}
                  style={{ height: `${height}%` }}
                  animate={activeVersion === 'new' && isPlaying ? {
                    scaleY: [1, 1.15, 1]
                  } : {}}
                  transition={{ duration: 0.3, delay: i * 0.01 }}
                />
