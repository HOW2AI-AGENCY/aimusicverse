...
export const LyricsRenderer = React.forwardRef<HTMLDivElement, LyricsRendererProps>(
  ({ lines, currentTime, isPlaying, onWordClick, highlightedSection, className }, ref) => {
    const renderWord = (word: AlignedWord, lineStartTime: number) => {
      ...
      const isHighlighted =
        highlightedSection && word.startS >= highlightedSection.start && word.endS <= highlightedSection.end;
    ...
    return (
      <div ref={ref} className={cn("space-y-3 py-4", className)}>
        {lines.map((line, lineIndex) => {
          ...
          const isHighlighted =
            highlightedSection && line.startTime >= highlightedSection.start && line.endTime <= highlightedSection.end;
