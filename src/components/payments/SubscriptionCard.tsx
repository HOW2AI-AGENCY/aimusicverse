...
export function SubscriptionCard({
  ...
  const pricePerDay =
    product.price_stars && product.subscription_days
    ...
    >
      ...
      <CardHeader className="space-y-4 pt-8">
        ...
        <motion.div
          ...
          className={cn("mx-auto w-16 h-16 rounded-2xl bg-gradient-to-br p-0.5", config.gradient)}
        >
        ...
        {/* Description */}
        {product.description && (
          <p className="text-sm text-muted-foreground text-center line-clamp-2">{product.description}</p>
        )}
        ...
        <div className="text-center pt-4 pb-2">
          ...
          <div className="mt-2 space-y-1">
            <p className="text-sm text-muted-foreground">за {product.subscription_days} дней</p>
            {pricePerDay && <p className="text-xs text-muted-foreground/70">≈ {pricePerDay} ⭐/день</p>}
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="pt-0 flex-1">
        ...
        {product.credits_amount && (
          <div className={cn("flex items-center justify-between p-4 rounded-xl mb-4", gradientGlass.primary)}>
      ...
      <CardFooter className="pt-6 pb-6">
        <StarsPaymentButton
          ...
          size="lg"
          className={cn("w-full font-semibold", product.is_featured && "shadow-lg shadow-primary/25")}
        >
          {isCurrentTier ? (
            <>
