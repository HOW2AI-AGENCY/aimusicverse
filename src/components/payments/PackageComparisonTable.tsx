  ...
  return (
    <div className={cn('space-y-3', className)}>
      {products.map((product, index) => {
        ...
        const basePricePerCredit = products[0]?.price_rub_cents && baseCredits 
          ? (products[0].price_rub_cents / 100) / baseCredits 
          : 0;
        const currentPricePerCredit = product.price_rub_cents && product.credits_amount
          ? (product.price_rub_cents / 100) / product.credits_amount
          : 0;
        const savingsPercent = basePricePerCredit && currentPricePerCredit && basePricePerCredit > currentPricePerCredit
