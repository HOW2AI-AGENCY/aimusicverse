...
export function ProjectsTab({ onProjectSelect, selectedProjectId }: ProjectsTabProps) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        className={cn("flex items-center gap-2 text-xs", isMobile && "gap-1.5")}
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 0.2 }}
      >
        <div
          className={cn("flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-muted/50", isMobile && "px-2 py-1")}
        >
      ...
      <AlertDialog open={!!deleteConfirmId} onOpenChange={(open) => !open && setDeleteConfirmId(null)}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => deleteConfirmId && handleDelete(deleteConfirmId)} disabled={isDeleting}>
      ...
      <ProjectCreationWizard open={createSheetOpen} onOpenChange={setCreateSheetOpen} />
    </motion.div>
  );
}
