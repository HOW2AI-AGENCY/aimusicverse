...
export function LighthouseScoresCard({ metric, previousMetric, isLoading }: LighthouseScoresCardProps) {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        {scores.map((score) => {
          const diff = score.value && score.prevValue ? score.value - score.prevValue : 0;
            ...
            <div key={score.name} className="space-y-1">
              <div className="flex items-center justify-between text-sm">
                ...
                <div className="flex items-center gap-2">
                  <span className={`font-mono ${getScoreColor(score.value)}`}>{score.value ?? "—"}</span>
                  {diff !== 0 && (
              ...
              <Progress value={score.value ?? 0} className={`h-1.5 ${getProgressColor(score.value)}`} />
            </div>
          );
        })}

        <div className="pt-2 border-t text-xs text-muted-foreground">
          Target: Performance {">"} {PERFORMANCE_TARGETS.lighthousePerformance}, Accessibility {">"}{" "}
          {PERFORMANCE_TARGETS.lighthouseAccessibility}
        </div>
      </CardContent>
    </Card>
  );
