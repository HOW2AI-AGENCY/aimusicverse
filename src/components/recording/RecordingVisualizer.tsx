...
export const RecordingVisualizer = memo(function RecordingVisualizer({
  ...
  if (variant === "waveform" && waveformData.length > 0) {
    return (
      <div className={cn("flex items-center justify-center gap-px overflow-hidden", className)} style={{ height }}>
  ...
  if (variant === "combined") {
    return (
      <div className={cn("flex flex-col gap-2", className)}>
        {/* VU Meter Bars */}
        <div className="flex items-end justify-center gap-1" style={{ height: height * 0.6 }}>
          {barHeights.map((h, i) => (
            <motion.div
              key={i}
              className={cn("w-2 rounded-t-sm transition-colors", getLevelColor(audioLevel))}
              animate={{
                height: h * height * 0.6,
        ...
        {waveformData.length > 0 && (
          <div className="flex items-center justify-center gap-px overflow-hidden" style={{ height: height * 0.3 }}>
  ...
  return (
    <div className={cn("flex items-end justify-center gap-1", className)} style={{ height }}>
      {barHeights.map((h, i) => (
        <motion.div
          key={i}
          className={cn("w-2 sm:w-3 rounded-t-sm transition-colors duration-150", getLevelColor(audioLevel))}
          animate={{
            height: h * height,
            opacity: isRecording && !isPaused ? 1 : 0.3,
          }}
