  ...
  if (!isSupported) {
    return (
      <div className={cn("flex flex-col items-center justify-center gap-2 p-6 text-center", className)}>
        ...
        <p className="text-sm text-muted-foreground">Запись аудио не поддерживается в этом браузере</p>
      </div>
    );
  }
    ...
    return (
      <div className={cn("flex items-center gap-4", className)}>
        ...
        {audioBlob && !isRecording && (
          <span className="text-xs text-muted-foreground">Записано {formatDuration(duration)}</span>
        )}
      </div>
    );
  }
      ...
      <AnimatePresence>
        {isRecording && (
          ...
          >
            ...
            {maxDuration > 0 && <span className="text-sm text-muted-foreground">/ {formatDuration(maxDuration)}</span>}
            {isPaused && <span className="text-sm text-yellow-500 font-medium">(пауза)</span>}
          </motion.div>
        )}
      </AnimatePresence>
