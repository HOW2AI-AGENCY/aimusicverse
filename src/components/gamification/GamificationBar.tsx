...
export const GamificationBar = memo(function GamificationBar({ className }: GamificationBarProps) {
  ...
  if (!user || isLoading) {
    return <div className={cn("h-12 sm:h-14 bg-card/40 animate-pulse rounded-2xl", className)} />;
  }
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                ...
                <motion.div className="absolute inset-0 rounded-full bg-primary/20 blur-md opacity-0 group-hover:opacity-100 transition-opacity" />
              </motion.button>
            </TooltipTrigger>
...
const StatPill = memo(function StatPill({ icon, value, color, onClick, pulse, glow, tooltip }: StatPillProps) {
  const colorStyles = {
  ...
  const content = (
    ...
    >
      ...
      {pulse && <span className="absolute -top-0.5 -right-0.5 w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse" />}
    </motion.button>
  );
