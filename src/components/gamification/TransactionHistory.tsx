  ...
  if (!transactions || transactions.length === 0) {
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-12">
        ...
        <p className="text-sm text-muted-foreground/70 mt-1">Выполняйте действия, чтобы зарабатывать кредиты</p>
      </motion.div>
    );
  }
          ...
          {Object.entries(groupedTransactions).map(([dateKey, dayTransactions], groupIndex) => (
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-muted-foreground">{dayTransactions.length} операц.</span>
              </div>
                      ...
                      <Card className="overflow-hidden border-0 shadow-sm">
                        <CardContent className="p-3">
                          <div className="flex items-center gap-3">
                                ...
                                {metadata?.experience_earned && (
                                  <>
                                    ...
                                    <span className="flex items-center gap-0.5 text-primary">
                                      <Sparkles className="w-3 h-3" />+{metadata.experience_earned} XP
                                    </span>
                                  </>
                                )}
                              </div>
                            ...
                            >
                              <Badge variant={isEarn ? "default" : "destructive"} className="gap-1 font-bold">
