...
export const AppLogo = memo(function AppLogo({
  ...
  return (
    <div className={cn("flex flex-col items-center", className)} style={style}>
      ...
      <div className={cn("relative", variant === "glow" && "p-3 bg-card/80 backdrop-blur-sm border border-primary/20")}>
        {/* Glow effect */}
        {variant === "glow" && (
          <div
      ...
      {/* App name */}
      {showName && <h1 className={cn(sizes.text, "font-bold text-gradient leading-tight mt-1")}>MusicVerse AI</h1>}

      {/* Tagline */}
      {showTagline && <p className={cn(sizes.tagline, "text-muted-foreground")}>Генерация музыки с AI</p>}
    </div>
  );
});
