...
function calculateForecast(
  ...
  const previousAvg =
    previousDays.length > 0 ? previousDays.reduce((sum, d) => sum + d.usd, 0) / previousDays.length : recentAvg;
...
export function RevenueForecast() {
  ...
  const projectionStartDate = projectionStartIndex > 0 ? forecast[projectionStartIndex - 1]?.day : null;
    ...
    <Card>
      ...
      <CardContent className="px-3 pb-3 sm:px-6 sm:pb-6 space-y-4">
        ...
        {isLoading ? (
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={forecast} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              {/* Confidence interval */}
              <Area type="monotone" dataKey="upper" stroke="none" fill="url(#colorConfidence)" fillOpacity={1} />
        ...
        <div className="flex items-center justify-center gap-4 sm:gap-6 text-[10px] sm:text-xs text-muted-foreground">
          ...
          <div className="flex items-center gap-1.5">
            <div
              className="w-3 h-0.5 bg-[hsl(var(--chart-2))] rounded"
              style={{
                backgroundImage:
                  "repeating-linear-gradient(90deg, transparent, transparent 2px, hsl(var(--chart-2)) 2px, hsl(var(--chart-2)) 4px)",
              }}
            />
            <span>Прогноз</span>
          </div>
