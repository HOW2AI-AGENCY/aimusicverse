  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<ForecastData> => {
      ...
      return {
        ...
        tracks: tracksDaily,
        predictions: {
          usersNext30: forecast(usersDaily.map(d => d.value), forecastDays),
          revenueNext30: forecast(revenueDaily.map(d => d.value), forecastDays),
          generationsNext30: forecast(generationsDaily.map(d => d.value), forecastDays),
          tracksNext30: forecast(tracksDaily.map(d => d.value), forecastDays),
        },
        growth: {
            ...
            <CardContent className="p-3 sm:p-4">
              ...
              <div className="space-y-1">
                ...
                <div className={cn(
                  ...
                  <span>{f.growth > 0 ? '+' : ''}{f.growth.toFixed(1)}%</span>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="p-3 rounded-lg bg-muted/50 border">
            ...
            <ul className="mt-2 space-y-1 text-sm">
              <li className="flex items-center gap-2">
                <Badge variant="outline" className="text-xs">+{Math.round(data?.predictions.usersNext30 || 0)}</Badge>
                новых пользователей
              </li>
              <li className="flex items-center gap-2">
                <Badge variant="outline" className="text-xs">⭐ {Math.round(data?.predictions.revenueNext30 || 0)}</Badge>
                дохода в Stars
              </li>
              <li className="flex items-center gap-2">
                <Badge variant="outline" className="text-xs">+{Math.round(data?.predictions.generationsNext30 || 0)}</Badge>
                генераций
              </li>
              <li className="flex items-center gap-2">
                <Badge variant="outline" className="text-xs">+{Math.round(data?.predictions.tracksNext30 || 0)}</Badge>
                новых треков
              </li>
            </ul>
