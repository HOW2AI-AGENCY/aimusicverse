...
export function DeeplinkTrendsChart({ data, timeRange, isLoading }: DeeplinkTrendsChartProps) {
  ...
  const chartData = useMemo(() => {
    return data.map((point) => ({
      ...
      conversionRate: point.visits > 0 ? Math.round((point.conversions / point.visits) * 100) : 0,
    }));
  }, [data]);
        ...
        <div className="h-[200px] sm:h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              ...
              <XAxis dataKey="dateFormatted" tick={{ fontSize: 10 }} tickLine={false} axisLine={false} />
