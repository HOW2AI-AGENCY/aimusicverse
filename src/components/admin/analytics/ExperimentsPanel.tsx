              ...
              <TabsContent key={tab} value={tab} className="space-y-4">
                ...
                {experiments.filter(exp => tab === 'all' || exp.status === tab).length === 0 && (
                  <div className="text-center py-8 text-muted-foreground">
                    Нет экспериментов в этой категории
                  </div>
                )}
              </TabsContent>
            ))}
...
function ExperimentCard({ experiment, result, isSelected, onSelect }: ExperimentCardProps) {
  ...
  const totalParticipants = result 
    ? Object.values(result.participants).reduce((a, b) => a + b, 0)
    : 0;
        ...
        {result?.winner && (
          <div className="text-right">
            <div className="text-sm font-medium text-green-500">
              +{result.uplift.toFixed(1)}% подъём
            </div>
        ...
        <div className="border-t pt-3 mt-3 space-y-3">
          ...
          {result && (
            <div className="flex items-center justify-between">
              ...
              <Badge className={cn(
                result.significance >= 0.95 ? 'bg-green-500' :
                result.significance >= 0.90 ? 'bg-yellow-500' : 'bg-red-500'
              )}>
