...
const JourneyCard = memo(function JourneyCard({ journey }: JourneyCardProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div className={cn("h-10 w-10 rounded-lg flex items-center justify-center shrink-0", outcomeStyle.bg)}>
                ...
                {journey.steps.map((step, index) => {
                  ...
                  return (
                    <div key={step.id} className="flex items-center">
                      ...
                      {!isLast && <ArrowRight className="h-4 w-4 text-muted-foreground mx-2 shrink-0" />}
                    </div>
                  );
                })}
              </div>
...
export const UserJourneyTimeline = memo(function UserJourneyTimeline() {
  ...
  const filteredJourneys = filter === "all" ? JOURNEYS : JOURNEYS.filter((j) => j.outcome === filter);
