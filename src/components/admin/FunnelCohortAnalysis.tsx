...
const CohortRow = memo(function CohortRow({ cohort, maxVisitors }: CohortRowProps) {
  ...
  const TrendIcon = cohort.trend === "up" ? TrendingUp : cohort.trend === "down" ? TrendingDown : null;
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-right shrink-0">
              <div
                className={cn(
                  "text-sm font-bold",
                  cohort.conversionRate >= 5
                    ? "text-green-500"
                    : cohort.conversionRate >= 3
                      ? "text-yellow-500"
                      : "text-red-500",
                )}
              >
...
export const FunnelCohortAnalysis = memo(function FunnelCohortAnalysis() {
  const maxVisitors = useMemo(() => Math.max(...COHORT_DATA.map((c) => c.visitors)), []);

  const sortedCohorts = useMemo(() => [...COHORT_DATA].sort((a, b) => b.conversionRate - a.conversionRate), []);
      ...
      <CardContent className="space-y-3">
        ...
        <div className="space-y-2">
          {COHORT_DATA.map((cohort) => (
            <CohortRow key={cohort.source} cohort={cohort} maxVisitors={maxVisitors} />
          ))}
        </div>
      </CardContent>
    </Card>
