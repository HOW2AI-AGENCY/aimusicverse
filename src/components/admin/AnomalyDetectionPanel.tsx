      ...
      <div className="flex items-start gap-2 sm:gap-3">
        ...
        <div className="flex-1 min-w-0 space-y-1">
          ...
          <div className="flex items-center gap-3 text-[0.625rem] sm:text-xs text-muted-foreground">
            ...
            <span>{new Date(anomaly.timestamp).toLocaleDateString("ru-RU")}</span>
          </div>
        </div>
      </div>
    </div>
      ...
      <CardHeader className="pb-2 sm:pb-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <CardTitle className="text-base sm:text-lg flex items-center gap-2">
            <AlertTriangle
              className={cn("h-4 w-4 sm:h-5 sm:w-5", hasCritical ? "text-red-500" : "text-muted-foreground")}
            />
            Детектор аномалий
            {hasAnomalies && (
              <Badge variant={hasCritical ? "destructive" : "secondary"} className="ml-2 text-xs">
                {anomalies.length}
              </Badge>
            )}
          </CardTitle>
      ...
      <CardContent className="space-y-3 sm:space-y-4">
        ...
        {!hasAnomalies ? (
          <div className="py-6 sm:py-8 text-center">
            ...
            <div className="text-sm sm:text-base font-medium text-green-500">Аномалий не обнаружено</div>
            <div className="text-xs sm:text-sm text-muted-foreground">Все метрики в пределах нормы</div>
          </div>
        ) : (
