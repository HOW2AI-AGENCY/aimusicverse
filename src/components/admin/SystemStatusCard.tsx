...
export function SystemStatusCard() {
  ...
  const config = healthStatus ? statusConfig[healthStatus.overall_status] : statusConfig.healthy;
        ...
        {healthStatus?.metrics && (
          <div className="grid grid-cols-2 gap-2 sm:gap-3">
            <div className="p-2 sm:p-3 rounded-lg bg-background/50">
              <div className="text-lg sm:text-xl font-bold">{healthStatus.metrics.active_generations}</div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-1.5 sm:gap-2">
              {Object.entries(healthStatus.checks).map(([key, check]) => {
                ...
                return (
                  <Badge key={key} variant="outline" className={cn("text-xs gap-1", checkConfig.color)}>
