  ...
  return (
    <div className="relative">
      ...
      <div className="relative h-8 sm:h-10 bg-muted/30 rounded-lg overflow-hidden">
        <div
          className={cn("h-full transition-all duration-500 rounded-lg", getBarColor())}
...
export function FunnelVisualization() {
  ...
  const startDate = useMemo(() => new Date(Date.now() - selectedRange * 24 * 60 * 60 * 1000), [selectedRange]);
          ...
          {isLoading
            ...
            : steps?.map((step, index) => (
                <FunnelBar
                  ...
                  isBiggestDropoff={metrics.biggestDropoff?.step === step.step_name && index > 0}
                />
              ))}
        </div>
        ...
        {!isLoading && metrics.biggestDropoff && metrics.biggestDropoff.rate > 50 && (
          <div className="bg-orange-500/10 border border-orange-500/20 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-medium text-orange-700 dark:text-orange-400">Обнаружен высокий отток</h4>
