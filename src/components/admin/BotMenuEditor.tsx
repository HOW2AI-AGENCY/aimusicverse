    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          {item.action_data && <span className="truncate max-w-[150px]">→ {item.action_data}</span>}
        </div>
      </div>

      <div className="flex items-center gap-2">
        <Switch checked={item.is_enabled} onCheckedChange={() => onToggle(item)} aria-label="Включить/выключить" />
...
export function BotMenuEditor() {
  ...
  const parentItems = useMemo(() => {
    return items.filter((item) => item.parent_key === null || item.action_type === "submenu");
  }, [items]);
            ...
            <Card className="lg:col-span-3">
              <CardHeader className="pb-3">
                <CardTitle className="text-sm flex items-center gap-2">
                  ...
                  <span className="text-muted-foreground font-normal">{currentItems.length} пунктов</span>
                </CardTitle>
              </CardHeader>
              <CardContent className="p-2">
                <ScrollArea className="h-[400px]">
                  <div className="space-y-2 pr-4">
                    {currentItems.length === 0 ? (
                      <div className="text-center py-8 text-muted-foreground">
                        ...
                        <Button variant="outline" className="mt-2" onClick={handleCreate}>
                          <Plus className="h-4 w-4 mr-2" />
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {getChildCount(deleteItem?.menu_key || "") > 0 && (
                <span className="text-destructive block mt-2">Внимание: у этого пункта есть вложенные элементы!</span>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
