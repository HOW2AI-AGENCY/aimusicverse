...
export const LyricsPresetsRow = memo(function LyricsPresetsRow({
  ...
  const visiblePresets = variant === "chips" ? getQuickLyricsPresets() : LYRICS_PRESETS.slice(0, maxPresets);
      ...
      <div className="relative -mx-4 px-4">
        <div className="flex gap-2 overflow-x-auto scrollbar-hide snap-x snap-mandatory pb-1">
          {visiblePresets.map((preset, index) =>
            variant === "chips" ? (
              <LyricsChip key={preset.id} preset={preset} index={index} onClick={() => handlePresetClick(preset)} />
            ) : (
              <LyricsCard key={preset.id} preset={preset} index={index} onClick={() => handlePresetClick(preset)} />
            ),
          )}
          ...
          <motion.button
            ...
            className={cn(
              variant === "chips" ? "flex-shrink-0 px-3 py-2 rounded-full" : "flex-shrink-0 w-[80px] p-2.5 rounded-xl",
