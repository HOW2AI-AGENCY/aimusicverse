...
export function ProgressTracker({ steps, orientation = "horizontal", compact = false }: ProgressTrackerProps) {
        ...
        {steps.map((step, index) => (
          <div key={step.id} className="flex items-start gap-3">
            ...
            <div className="flex-1 pt-1">
              <p
                className={`text-sm font-medium ${step.current ? "text-primary" : step.completed ? "text-foreground" : "text-muted-foreground"}`}
              >
                {step.label}
              </p>
            </div>
      ...
      <div className="flex items-center gap-1 overflow-x-auto pb-2">
        {steps.map((step, index) => (
          <div key={step.id} className="flex items-center shrink-0">
            {index > 0 && <div className={`w-12 h-0.5 ${step.completed ? "bg-primary" : "bg-muted"}`} />}
            <div className="flex flex-col items-center gap-1">
              ...
              <span
                className={`text-xs text-center max-w-[80px] ${step.current ? "font-medium text-primary" : "text-muted-foreground"}`}
              >
                {step.label}
              </span>
            </div>
