...
const PatternCard = memo(function PatternCard({ pattern, onLoad }: PatternCardProps) {
  ...
  const totalSteps = Object.values(pattern.steps).reduce((acc, steps) => acc + steps.filter(Boolean).length, 0);
    ...
    >
      ...
      <div className="flex gap-px h-8 w-full overflow-hidden rounded-md bg-muted/30">
        {Array.from({ length: 16 }, (_, step) => {
          const hasHit = Object.values(pattern.steps).some((steps) => steps[step]);
          return (
            <div
              key={step}
              className={cn(
      ...
      <div className="h-1 w-full bg-muted/30 rounded-full overflow-hidden">
        <div className="h-full bg-primary/50 rounded-full transition-all" style={{ width: `${density}%` }} />
      </div>
    </button>
  );
});
          ...
          <ScrollArea className="w-full">
            <div className="flex gap-3 pb-2">
              {patterns.map((pattern) => (
                <PatternCard key={pattern.id} pattern={pattern} onLoad={() => onLoadPattern(pattern)} />
              ))}
            </div>
