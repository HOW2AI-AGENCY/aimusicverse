...
export const DrumMachine = memo(function DrumMachine({ className }: DrumMachineProps) {
  ...
  return (
    <div className={cn("flex flex-col gap-3 p-3 bg-card rounded-xl border", className)} onClick={handleInitialize}>
      {/* Header */}
      <div className="flex items-center justify-between flex-wrap gap-2">
        <h3 className="text-base font-semibold flex items-center gap-2">
          🥁 Драм-машина
          {!isReady && (
            <span className="text-xs font-normal text-muted-foreground animate-pulse">(нажмите для активации)</span>
          )}
        </h3>
      ...
      {/* Kit selector */}
      <DrumKitSelector kits={getAvailableKits()} currentKitId={currentKit.id} onSelectKit={setKit} />
      ...
      {/* Main view */}
      {viewMode === "pads" ? (
        <DrumPadGrid sounds={currentKit.sounds} onPadTrigger={handlePadTrigger} activePads={activePads} />
      ) : (
        <DrumSequencer
      ...
      <div className="flex items-center gap-3 pt-2 border-t flex-wrap">
        ...
        <DrumEffects sounds={currentKit.sounds} trackEffects={trackEffects} onSetEffect={setTrackEffect} />
      ...
      <div className="flex items-center gap-2 pt-2 border-t">
        <Button variant="secondary" size="sm" onClick={handleSendToPromptDJ} className="h-8 gap-1.5 flex-1">
      ...
      {/* Pattern bank */}
      <PatternBank patterns={getPresetPatterns()} onLoadPattern={loadPattern} onClearPattern={clearPattern} />
    </div>
  );
});
