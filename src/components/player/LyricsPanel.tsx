...
import { useLyricsSynchronization, SYNC_CONSTANTS, type AlignedWord } from "@/hooks/lyrics/useLyricsSynchronization";
  ...
  return (
    <div className={cn("relative flex flex-col h-full", className)}>
      ...
      <div
        ...
        className={cn("flex-1 overflow-y-auto px-4 py-4 lyrics-scroll-container", isMobile && "touch-action-pan-y")}
      ...
      >
        {lyricsLines ? (
          ...
          >
            {lyricsLines.map((line, lineIndex) => {
              ...
              return (
                ...
                >
                  <div
                    className={cn("flex flex-wrap justify-center gap-x-1.5 gap-y-0.5", !isMobile && "gap-x-2 gap-y-1")}
                  >
                    {showWordHighlight ? (
          ...
          <div className="min-h-full flex items-center justify-center">
            <p className={cn("text-center leading-relaxed whitespace-pre-wrap text-foreground/90", textSize)}>
              {plainLyrics}
            </p>
          </div>
        ) : (
