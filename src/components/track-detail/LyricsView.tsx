...
export function LyricsView({ lyrics, currentTime = 0, onSeek, isLoading = false }: LyricsViewProps) {
  ...
  const lyricsData = typeof lyrics === "string" ? { normalLyrics: lyrics } : lyrics || {};
  ...
  useEffect(() => {
    ...
    const currentWord = lyricsData.alignedWords?.find((word) => currentTime >= word.startS && currentTime <= word.endS);
  ...
  if (!lyricsText && !hasTimestampedLyrics) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-center p-4">
        <p className="text-muted-foreground text-lg mb-2">Текст песни недоступен</p>
        <p className="text-muted-foreground text-sm">Текст появится здесь после генерации</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      >
        {hasTimestampedLyrics ? (
                  ...
                  >
                    {line.map((word, wordIndex) => {
                      ...
                      return (
                        <span
                          key={wordIndex}
                          className={cn("mr-2 transition-colors", isWordActive && "text-primary font-extrabold")}
                        >
                          {word.word.replace("\n", "")}
                        </span>
                      );
          ...
          <div className="whitespace-pre-wrap text-base leading-relaxed text-foreground">{lyricsText}</div>
        )}
      </div>
    </div>
  );
