...
const StatCard = memo(function StatCard({ icon, label, value, color, subValue }: StatCardProps) {
  ...
  return (
    <Card className={cn("p-3 sm:p-4 border bg-gradient-to-br transition-all hover:scale-[1.02]", color)}>
...
export const AnalysisQuickStats = memo(function AnalysisQuickStats({ analysis }: AnalysisQuickStatsProps) {
  ...
  const stats = [
    ...
    },
    {
      ...
      label: "Энергия",
      value:
        normalized.arousal !== null && normalized.arousal !== undefined ? `${Math.round(normalized.arousal)}%` : null,
    ...
    },
    {
      ...
      label: "Позитив",
      value:
        normalized.valence !== null && normalized.valence !== undefined ? `${Math.round(normalized.valence)}%` : null,
