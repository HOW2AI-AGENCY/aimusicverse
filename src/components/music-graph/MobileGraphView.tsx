...
export function MobileGraphView({ data, onNodeClick, selectedNode, filter }: MobileGraphViewProps) {
  ...
  const currentItems = useMemo(() => {
    if (drillLevel === "items" && selectedGroup) {
      const items =
        selectedCluster === "genres"
          ? groupedData.genres[selectedGroup] || []
          : groupedData.categories[selectedGroup] || [];

      if (searchQuery) {
        return items.filter((n) => n.label.toLowerCase().includes(searchQuery.toLowerCase()));
      }
      return items;
    }
    return [];
    ...
    <div className="grid grid-cols-2 gap-3 p-4">
      ...
      <div className="col-span-2 p-4 rounded-xl bg-card border border-border">
        ...
        <div className="grid grid-cols-4 gap-2 text-center">
          <StatItem count={filteredData.nodes.filter((n) => n.type === "genre").length} label="Жанров" />
          <StatItem count={filteredData.nodes.filter((n) => n.type === "style").length} label="Стилей" />
          <StatItem count={filteredData.nodes.filter((n) => n.type === "tag").length} label="Тегов" />
          <StatItem count={filteredData.edges.length} label="Связей" />
        </div>
      </div>
    </div>
  );
  ...
  const renderCluster = () => {
    const groups = selectedCluster === "genres" ? groupedData.genres : groupedData.categories;
        ...
        <div className="space-y-2 pr-4">
          {currentItems.map((node) => (
            <NodeCard
              ...
              relatedCount={data.edges.filter((e) => e.source === node.id || e.target === node.id).length}
            />
          ))}
        </div>
      </ScrollArea>
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{NODE_TYPE_LABELS[selectedNode.type]}</p>
                  </div>
                </div>
                <Button variant="ghost" size="icon" onClick={() => onNodeClick?.(selectedNode)}>
                  <X className="w-4 h-4" />
                </Button>
              </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-muted-foreground truncate">{samples.join(", ")}...</p>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-muted-foreground">{relatedCount} связей</p>
      </div>
      <ChevronRight className="w-4 h-4 text-muted-foreground flex-shrink-0" />
    </motion.button>
  );
