              ...
              {showMobileSidebarToggle && (
                <Sheet>
                  ...
                  <SheetContent side="bottom" className="h-[70vh]">
                    <div className="overflow-auto h-full pt-4">{sidebar}</div>
                  </SheetContent>
                </Sheet>
              )}
            </div>
    ...
    <div className={cn("w-full mx-auto", MAX_WIDTHS[maxWidth], className)}>
      ...
      <div className="flex min-h-[calc(100vh-120px)]">
        ...
        {sidebar && sidebarPosition === "left" && (
          <aside className={cn("shrink-0 border-r overflow-auto", sidebarWidth)}>
            <div className="p-4 lg:p-6 sticky top-0">{sidebar}</div>
          </aside>
        )}
        ...
        {sidebar && sidebarPosition === "right" && (
          <aside className={cn("shrink-0 border-l overflow-auto bg-muted/30", sidebarWidth)}>
            <div className="p-4 lg:p-6 sticky top-0">{sidebar}</div>
          </aside>
        )}
      </div>
