...
import type { StudioTrack, StudioClip, TrackType, TrackStatus, StudioTrackVersion } from "./types";
...
export const useTrackStore = create<TrackState>()(
  subscribeWithSelector((set, get) => ({
    ...
    updatePendingTrackTaskId: async (trackId: string, taskId: string) => {
      set({
        tracks: get().tracks.map((t) => (t.id === trackId ? { ...t, taskId } : t)),
      });
          ...
          if (t.id === trackId && t.versions) {
            ...
            if (version) {
              return {
                ...
                clips: t.clips.map((clip, i) => (i === 0 ? { ...clip, audioUrl: version.audioUrl } : clip)),
              };
            }
          }
          return t;
          ...
          if (t.id === trackId) {
            return {
              ...
              clips:
                t.clips.length > 0
                  ? t.clips.map((clip, i) =>
                      i === 0 ? { ...clip, audioUrl, duration: duration || clip.duration } : clip,
                    )
                  : [
                      {
                        id: generateId(),
    ...
    moveClip: (clipId, newTrackId, newStartTime) => {
      ...
      set({
        tracks: tracksWithoutClip.map((t) => (t.id === newTrackId ? { ...t, clips: [...t.clips, clipToMove!] } : t)),
      });
