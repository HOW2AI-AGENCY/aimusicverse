...
} from "@/components/gamification/UnifiedRewardNotification";
...
export function RewardNotificationProvider({ children }: { children: ReactNode }) {
  ...
  const showLevelUp = useCallback(
    (level: number, title?: string, rewards?: { credits?: number; experience?: number }) => {
      setNotificationData({
        type: "level_up",
        level,
        levelTitle: title,
  ...
  const showAchievement = useCallback(
    (name: string, description?: string, icon?: string, rewards?: { credits?: number; experience?: number }) => {
      setNotificationData({
        type: "achievement",
  ...
  const showStreak = useCallback((streak: number, rewards?: { credits?: number; experience?: number }) => {
    setNotificationData({
      type: "streak",
      streak,
      credits: rewards?.credits,
  ...
  const showSubscription = useCallback((tier: "pro" | "premium", credits: number, features?: string[]) => {
    setNotificationData({
      type: "subscription",
  ...
  return (
    <RewardNotificationContext.Provider value={contextValue}>
      ...
      <UnifiedRewardNotification data={notificationData} onComplete={hide} />
