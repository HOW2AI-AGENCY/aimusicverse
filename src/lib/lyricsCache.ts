 ...
 */
export async function getCachedLyrics(taskId: string, audioId: string): Promise<unknown | null> {
  try {
 ...
 */
export async function setCachedLyrics(taskId: string, audioId: string, data: unknown): Promise<void> {
  try {
    const db = await getDB();
    const key = getCacheKey(taskId, audioId);
 ...
 */
export async function deleteCachedLyrics(taskId: string, audioId: string): Promise<void> {
  try {
    const db = await getDB();
    const key = getCacheKey(taskId, audioId);
