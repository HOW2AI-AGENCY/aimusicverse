...
function findBestEndPosition(
  ...
  for (let offset = 0; offset <= searchRadius; offset++) {
    for (const direction of [0, 1, -1]) {
      ...
      const score = scoreSequenceMatch(lastWords, alignedWords, i - WINDOW_SIZE + 1, WINDOW_SIZE);
...
export function alignSectionBoundaries(
  ...
  const { snapToWordStart = true, snapToWordEnd = true, expandToPhrase = false } = options;
...
export function extractLyricsForTimeRange(
  ...
  const words = alignedWords.filter((w) => w.startS >= startTimeS - 0.1 && w.endS <= endTimeS + 0.1);
