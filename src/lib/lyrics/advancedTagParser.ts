...
export function isStructuralTagValue(value: string): boolean {
  ...
  return STRUCTURAL_PATTERNS.some(
    (p) => base === p.toLowerCase() || base.replace(/[^a-zа-яё]/gi, "") === p.replace(/[^a-zа-яё]/gi, ""),
  );
}
 ...
 */
export function parseTag(raw: string, format: "bracket" | "parenthesis", startIndex: number): ParsedTag {
  // Extract inner content
  const inner = format === "bracket" ? raw.slice(1, -1) : raw.slice(1, -1);
