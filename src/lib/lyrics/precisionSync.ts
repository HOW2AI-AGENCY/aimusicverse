    ...
    if (shouldBreak || i === words.length - 1) {
      if (currentWords.length > 0) {
        ...
        currentWords.forEach((w, idx) => {
          w.positionInLine = currentWords.length > 1 ? idx / (currentWords.length - 1) : 0.5;
        });
...
export class TimeSmootherAdvanced {
  ...
  update(newTime: number): number {
    ...
    if (expectedDelta > 0) {
      ...
      this.velocity =
        this.velocity * PRECISION_SYNC.VELOCITY_SMOOTHING + currentVelocity * (1 - PRECISION_SYNC.VELOCITY_SMOOTHING);
    }
