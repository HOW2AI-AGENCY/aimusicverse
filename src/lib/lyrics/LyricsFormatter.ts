...
export class LyricsFormatter {
  ...
  static formatFinal(sections: LyricSection[], enrichment: EnrichmentTags): string {
    ...
    if (enrichment.vocalTags.length > 0) {
      const validTags = enrichment.vocalTags.map((t) => wrapInBrackets(t)).filter((t) => t.length > 0);
    ...
    sections.forEach((section, index) => {
      if (section.content.trim()) {
        ...
        if (section.tags.length > 0) {
          const validSectionTags = section.tags.map((t) => wrapInParentheses(t)).filter((t) => t.length > 0);
    ...
    if (enrichment.instrumentTags.length > 0) {
      const validInstrumentTags = enrichment.instrumentTags.map((t) => wrapInBrackets(t)).filter((t) => t.length > 0);
  ...
  static calculateCharCount(lyrics: string, excludeTags: boolean = true): number {
    if (excludeTags) {
      ...
      let lyricsWithoutTags = lyrics.replace(/\[.*?\]/g, "").replace(/\(.*?\)/g, "");
  ...
  static extractSections(lyrics: string): LyricSection[] {
    ...
    while ((match = sectionRegex.exec(lyrics)) !== null) {
      ...
      sections.push({
        id: `section-${index++}`,
        type: name.toLowerCase().includes("verse")
          ? "verse"
