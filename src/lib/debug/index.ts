 ...
 */
export function debugLog(tag: string, message: string, data?: Record<string, unknown>): void {
  if (!import.meta.env.DEV) return;

  console.log(`%c[${tag}]%c ${message}`, "color: #6366f1; font-weight: bold;", "color: inherit;", data || "");
}
 ...
 * @param data - Optional data to include
 */
export function debugWarn(tag: string, message: string, data?: Record<string, unknown>): void {
  if (!import.meta.env.DEV) return;
