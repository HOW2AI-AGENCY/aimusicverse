...
export const glass = {
  ...
  light: cn("bg-white/60 dark:bg-black/40", "backdrop-blur-md", "border border-white/20 dark:border-white/10"),

  // Medium glass - standard frosted glass
  medium: cn("bg-white/70 dark:bg-black/50", "backdrop-blur-lg", "border border-white/30 dark:border-white/15"),

  // Heavy glass - strong frosted effect
  heavy: cn("bg-white/80 dark:bg-black/60", "backdrop-blur-xl", "border border-white/40 dark:border-white/20"),
  ...
  player: cn("bg-card/95 backdrop-blur-xl", "border border-border/50 rounded-2xl", "shadow-lg shadow-black/10"),

  // Pill glass - for badges/pills
  pill: cn("bg-black/40 dark:bg-white/10", "backdrop-blur-sm", "border-0"),

  // Subtle glass - minimal effect
  subtle: cn("bg-muted/30 backdrop-blur-sm", "border border-border/30"),

  // Elevated glass - higher elevation
  elevated: cn("bg-card/85 backdrop-blur-xl", "border border-border/40", "shadow-xl shadow-black/10"),
  ...
  dropdown: cn("bg-popover/95 backdrop-blur-xl", "border border-border/50", "shadow-xl shadow-black/15"),
} as const;
...
export function createGlass(config: {
  ...
  const { blur: blurLevel = "lg", opacity = 0.7, border = true, shadow = true, dark = false } = config;

  const opacityPercent = Math.round(opacity * 100);

  return cn(
    dark ? `bg-black/${opacityPercent}` : `bg-card/${opacityPercent}`,
