 ...
 */
export function memoize<T extends (...args: any[]) => any>(fn: T, getCacheKey?: (...args: Parameters<T>) => string): T {
 ...
 */
export function batchUpdates<T>(callback: (updates: T[]) => void, delay: number = 16): (update: T) => void {
  let updates: T[] = [];
  let timeoutId: NodeJS.Timeout | null = null;
 ...
 */
export function monitorMemoryUsage(thresholdMB: number, callback: (usage: number) => void): () => void {
