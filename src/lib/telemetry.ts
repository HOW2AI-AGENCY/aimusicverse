 ...
 * Track feature usage
 */
export function trackFeature(feature: string, action: FeatureUsage["action"], metadata?: Record<string, unknown>) {
  if (!shouldSample()) return;
...
export async function flushMetrics(useBeacon = false) {
  ...
  if (useBeacon && navigator.sendBeacon) {
    ...
    try {
      ...
      navigator.sendBeacon(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/telemetry`, blob);
    } catch {
      // Silently fail
    }
    return;
 ...
 */
export function withTiming<T extends (...args: unknown[]) => Promise<unknown>>(name: string, fn: T): T {
...
export const generationAnalytics = {
  ...
  trackError: (mode: string, errorCode: string, context?: Record<string, unknown>) => {
    ...
    const errorMessage = context?.originalError ? String(context.originalError) : `Generation failed: ${errorCode}`;
...
export const playerAnalytics = {
  ...
  trackComplete: (trackId: string, listenDurationSec: number, totalDuration?: number) => {
    const completionRate =
      totalDuration && totalDuration > 0 ? Math.round((listenDurationSec / totalDuration) * 100) : undefined;
