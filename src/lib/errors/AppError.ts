...
const ERROR_METADATA: Record<ErrorCode, ErrorMetadata> = {
  ...
  [ErrorCode.TIMEOUT_ERROR]: {
    severity: ErrorSeverity.MEDIUM,
    recoveryStrategy: RecoveryStrategy.RETRY_BACKOFF,
    retryable: true,
  },
  ...
  [ErrorCode.INSUFFICIENT_CREDITS]: {
    ...
    retryable: false,
    userActionRequired: "Пополните баланс",
  },
  ...
  [ErrorCode.QUOTA_EXCEEDED]: {
    ...
    recoveryStrategy: RecoveryStrategy.MANUAL,
    retryable: false,
    userActionRequired: "Освободите место",
  },
...
export class ValidationError extends AppError {
  ...
  constructor(message: string, errors: string[] = [], field?: string, context?: Record<string, unknown>) {
...
export class AudioError extends AppError {
  constructor(message: string, code: ErrorCode = ErrorCode.AUDIO_PLAYBACK_ERROR, context?: Record<string, unknown>) {
...
export async function retryWithBackoff<T>(
  ...
  const { maxRetries = 3, initialDelayMs = 1000, maxDelayMs = 30000, shouldRetry = (e) => e.isRetryable() } = options;
