...
export const ERROR_CODE_MESSAGES: Record<string, ErrorCodeInfo> = {
  ...
  COPYRIGHTED_CONTENT: {
    ...
    examples: ["Используйте собственные метафоры и образы", "Опишите идею песни, а не копируйте существующие тексты"],
  },
  MALFORMED_LYRICS: {
  ...
  AUDIO_GENERATION_FAILED: {
    ...
    examples: ["Модель V4 лучше для сложных текстов", "V5 даёт лучшее качество для инструментальных треков"],
    retryDelayMs: 3000,
  },
  INTERNAL_ERROR: {
  ...
  EXTEND_LYRICS_EMPTY: {
    ...
    examples: ["[Continue]\nПродолжение истории...", 'Или включите "Инструментал" для трека без слов'],
  },
...
function detectErrorCodeFromMessage(message: string): string | null {
  const patterns: Array<{ match: (msg: string) => boolean; code: string }> = [
    ...
    {
      match: (msg) => msg.includes("internal error") || msg.includes("please try again later"),
      code: "INTERNAL_ERROR",
    },
    ...
    {
      match: (msg) =>
        msg.includes("extending lyrics empty") ||
        msg.includes("lyrics malformed") ||
        msg.includes("too short, or malformed"),
      code: "EXTEND_LYRICS_EMPTY",
    },
    {
      match: (msg) => msg.includes("artist name") || msg.includes("don't reference specific artists"),
      code: "ARTIST_NAME_NOT_ALLOWED",
    },
    {
      match: (msg) => msg.includes("matches existing work") || msg.includes("existing work of art"),
      code: "EXISTING_WORK_MATCHED",
    },
    { match: (msg) => msg.includes("uploaded audio") && msg.includes("protected"), code: "COVER_PROTECTED_CONTENT" },
    {
      match: (msg) =>
        msg.includes("server") && (msg.includes("overload") || msg.includes("busy") || msg.includes("500")),
...
export function showGenerationError(error: unknown): void {
  ...
  if (errorInfo) {
    ...
    const description = errorInfo.hint ? `${errorInfo.description} ${errorInfo.hint}` : errorInfo.description;
...
const FALSE_POSITIVE_WORDS = [
  ...
  "королева",
  "королеве",
  "королеву", // "queen" is too common
  "drake",
  "дракон",
  "драконе", // common word for "дракон" context
  "future",
  "будущее",
  "будущего", // too common
  "teni",
  ...
  "tena",
  "хаски",
  "husky", // husky is a dog breed
  "nervy",
  "нервы",
  "нерв", // common word "nerves"
  "mot",
  "мот",
  "мотор", // can be common word
  "face",
  "фэйс",
  "фейс",
  "лицо", // common word
  ...
  "томас мраз",
  "сектор газа",
  "sektor gaza", // only block if explicitly "in style of"
  ...
  "omens",
  "знамения",
  "предзнаменования", // common word "omens"
];
...
export function showErrorWithRecovery(error: unknown): void {
  ...
  const toastFn =
    severity === ErrorSeverity.FATAL || severity === ErrorSeverity.HIGH
      ? toast.error
