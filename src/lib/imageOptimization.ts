...
export function getTrackCoverUrl(
  ...
  if (thumbnails) {
    const thumbnailUrl =
      size === "small" ? thumbnails.small_url : size === "medium" ? thumbnails.medium_url : thumbnails.large_url;
 ...
 */
export function getThumbnailPlaceholderColor(thumbnails?: ThumbnailUrls | null): string {
 ...
 */
export function generatePlaceholder(width: number, height: number, color = "#1a1a2e"): string {
 ...
 */
export function generateBlurPlaceholder(width = 32, height = 32, baseColor = "hsl(240, 30%, 12%)"): string {
  const svg = `
