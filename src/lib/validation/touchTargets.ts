...
export function validateTouchTarget(
  ...
  if (opts.requireSquare && width !== height) {
    result.valid = false;
    result.errors.push(`${context}Touch target must be square: ${width}x${height}px. Use same width and height.`);
  }
  ...
  if (minDimension < 56 && result.valid) {
    result.recommendations.push(`Consider increasing to 56x56px for improved accessibility and easier tapping`);
  }
