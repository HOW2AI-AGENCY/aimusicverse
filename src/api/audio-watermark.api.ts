...
export async function watermarkAudio(options: WatermarkOptions): Promise<WatermarkResult> {
  const { data, error } = await supabase.functions.invoke<WatermarkResult>("replicate-audio-watermark", {
    body: {
...
export async function hasWatermark(trackId: string): Promise<boolean> {
  const { data } = await supabase.from("tracks").select("is_watermarked").eq("id", trackId).single();
...
export async function getWatermarkedUrl(trackId: string): Promise<string | null> {
  const { data } = await supabase.from("tracks").select("watermarked_url").eq("id", trackId).single();
