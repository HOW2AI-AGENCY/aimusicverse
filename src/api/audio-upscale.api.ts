...
export async function upscaleAudio(options: UpscaleOptions): Promise<UpscaleResult> {
  const { data, error } = await supabase.functions.invoke<UpscaleResult>("replicate-audio-upscale", {
    body: {
...
export async function hasHdAudio(trackId: string): Promise<boolean> {
  const { data } = await supabase.from("tracks").select("audio_url_hd, audio_quality").eq("id", trackId).single();
...
export async function getHdAudioUrl(trackId: string): Promise<string | null> {
  const { data } = await supabase.from("tracks").select("audio_url_hd").eq("id", trackId).single();
...
export async function getUpscaleStatus(trackId: string): Promise<string | null> {
  const { data } = await supabase.from("tracks").select("upscale_status").eq("id", trackId).single();
