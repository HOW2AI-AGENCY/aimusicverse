...
/**
 * Count the number of tracks owned by the user.
 */
export async function countUserTracks(userId: string): Promise<number> {
...
export async function toggleTrackLike(trackId: string, userId: string, isCurrentlyLiked: boolean): Promise<void> {
  if (isCurrentlyLiked) {
    ...
    const { error } = await supabase
      .from("track_likes")
      .insert({ track_id: trackId, user_id: userId } as unknown as { track_id: string; user_id: string; track_version_id: string });
    if (error) throw new Error(error.message);
  }
}
...
export async function fetchTrackVersionsDetailed(trackId: string): Promise<TrackVersionDetailRow[]> {
  const { data, error } = await supabase
    .from("track_versions")
    .select(
      "id, version_label, audio_url, cover_url, duration_seconds, is_primary, clip_index, version_type, created_at",
    )
    .eq("track_id", trackId)
