...
export function useTrackEnhancedData(trackIds: string[]) {
  ...
  const { data: versions } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase.from("track_versions").select("track_id").in("track_id", trackIds);
  ...
  const { data: stems } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase.from("track_stems").select("track_id").in("track_id", trackIds);
  ...
  const getMidiStatus = useCallback(
    (trackId: string): TrackMidiStatus | undefined => {
  ...
  const getEnhancedData = useCallback(
    (trackId: string): TrackEnhancedData => {
      return {
