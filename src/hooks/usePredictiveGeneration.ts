...
export function usePredictiveGeneration(currentPrompt: string, options: UsePredictiveGenerationOptions = {}) {
  const {
    enabled = true,
  ...
  const getPredictedTrack = useCallback(
    (prompt: string): PredictedTrack | null => {
  ...
  const preGenerate = useCallback(
    async (prompt: string, priority: number = 1) => {
      ...
      const activePredictions = Array.from(predictions.values()).filter((p) => p.status === "generating");
