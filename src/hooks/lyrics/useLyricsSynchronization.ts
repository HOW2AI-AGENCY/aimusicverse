 ...
 */
function findActiveWordIndex(words: AlignedWord[], time: number, lookAheadMs: number, endToleranceMs: number): number {
 ...
 */
function findActiveLineIndex(lines: LyricLine[], time: number, lookAheadMs: number, endToleranceMs: number): number {
...
export function useLyricsSynchronization({
  ...
  const updateSync = useCallback(() => {
    ...
    const interval = isPlaying ? SYNC_CONSTANTS.UPDATE_INTERVAL_PLAYING : SYNC_CONSTANTS.UPDATE_INTERVAL_PAUSED;
  ...
  const getEnhancedWord = useCallback(
    (index: number): EnhancedWord | null => {
