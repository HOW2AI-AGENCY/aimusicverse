...
export function useStudioEffectsEngine({ audioContext }: UseStudioEffectsEngineOptions): UseStudioEffectsEngineReturn {
  const effectsChainsRef = useRef<Map<string, EffectsChain>>(new Map());

  /**
   * Create complete effects chain for a track
   */
  const createEffectsChain = useCallback(
    (trackId: string): EffectsChain | null => {
      if (!audioContext) {
  ...
  const updateEffects = useCallback(
    (trackId: string, effects: StemEffects) => {
