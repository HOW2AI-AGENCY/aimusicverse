...
export function useMultiTrackAudio(sources: AudioSource[]) {
  ...
  const updateTime = useCallback(() => {
    ...
    const mostDrifted = audioWithDrift.reduce((max, current) => (current.drift > max.drift ? current : max));
  ...
  const skip = useCallback(
    (direction: "back" | "forward", amount = 10) => {
      const newTime =
        direction === "back" ? Math.max(0, currentTime - amount) : Math.min(duration, currentTime + amount);
      seek(newTime);
    },
