...
export function useStudioHandlers(options: StudioHandlersOptions) {
  const { id, patterns, operationLock, modals, modalHandlers, studioActions, trackForSeparation, separate } = options;
  ...
  const handleStemSeparationConfirm = useCallback(
    async (stemMode: "simple" | "detailed") => {
  ...
  const handleRecordingComplete = useCallback(
    async (recordedTrack: {
      ...
      try {
        ...
        const { error } = await supabase.from("track_stems").insert({
          track_id: id,
          stem_type: stemType,
