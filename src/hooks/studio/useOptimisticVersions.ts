...
export function useOptimisticVersions({ trackId, versions }: UseOptimisticVersionsProps) {
  ...
  const displayVersions = useMemo(() => {
    ...
    const pendingOptimistic = optimisticVersions.filter((ov) => !realVersionIds.has(ov.id));
   ...
   */
  const createVersionOptimistic = useCallback(
    async (audioUrl: string, metadata?: Record<string, unknown>) => {
   ...
   */
  const setPrimaryOptimistic = useCallback(
   ...
   */
  const deleteVersionOptimistic = useCallback(
    async (versionId: string) => {
      // Mark as pending
      setPendingOperations((prev) => new Set([...prev, versionId]));
