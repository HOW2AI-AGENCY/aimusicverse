...
import { defaultStemEffects, eqPresets, compressorPresets, reverbPresets } from "./stemEffectsConfig";
...
export function useStemStudioEngine(stemIds: string[]) {
  ...
  const initializeStemEngine = useCallback(
    async (stemId: string, audioElement: HTMLAudioElement) => {
  ...
  const updateStemReverb = useCallback(
    (stemId: string, settings: Partial<ReverbSettings>) => {
      const nodes = nodesMapRef.current[stemId];
      const ctx = audioContextRef.current;
      if (!nodes || !ctx) return;
  ...
  const applyStemEQPreset = useCallback(
    (stemId: string, presetName: keyof typeof eqPresets) => {
  ...
  const applyStemCompressorPreset = useCallback(
    (stemId: string, presetName: keyof typeof compressorPresets) => {
  ...
  // Apply Reverb preset
  const applyStemReverbPreset = useCallback(
    (stemId: string, presetName: keyof typeof reverbPresets) => {
  ...
  const resetStemEffects = useCallback(
    (stemId: string) => {
      updateStemEQ(stemId, {
        lowGain: 0,
        midGain: 0,
