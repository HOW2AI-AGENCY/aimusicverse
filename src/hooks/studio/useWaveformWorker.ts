...
function generatePeaksMainThread(audioData: Float32Array, targetPeaks: number): number[] {
...
export function useWaveformWorker(): UseWaveformWorkerReturn {
  ...
  const generatePeaks = useCallback(async (audioData: Float32Array, targetPeaks: number): Promise<number[]> => {
