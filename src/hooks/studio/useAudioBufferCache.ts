...
export function useAudioBufferCache(options: UseAudioBufferCacheOptions = {}) {
  ...
  const evictEntries = useCallback(
    (requiredSize: number) => {
      const entries = Array.from(globalCache.entries()).sort((a, b) => a[1].lastAccess - b[1].lastAccess);
  ...
  const set = useCallback(
    (url: string, buffer: AudioBuffer): void => {
  ...
  const preload = useCallback(
    (urls: string[]) => {
      ...
      const newUrls = urls.filter((url) => !has(url) && !preloadQueueRef.current.includes(url));
  ...
  const getStats = useCallback((): CacheStats => {
    ...
    const oldestEntry = entries.length > 0 ? Math.min(...entries.map((e) => e.lastAccess)) : 0;
