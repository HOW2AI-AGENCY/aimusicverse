...
export function useGenerationLogs(options: UseGenerationLogsOptions = {}) {
  ...
  useEffect(() => {
    const { unsubscribe } = generationApi.subscribeToGenerationLogs((payload) => {
      if (payload.eventType === "INSERT") {
        ...
        setRealtimeLogs((prev) => prev.map((log: GenerationLog) => (log.id === payload.new.id ? payload.new : log)));
      }
    });
  ...
  const mergedLogs = [...realtimeLogs, ...(logs || [])]
    .reduce((acc: GenerationLog[], log: GenerationLog) => {
