...
export const STEP_INDEX: Record<WizardStep, number> = {
  upload: 1, validating: 2, phrase_ready: 3, generating: 5, ready: 6, failed: 0,
};
...
export function useVoiceCloneWizard() {
  ...
  useEffect(() => () => { stopPolling(); stopRealtime(); }, [stopPolling, stopRealtime]);

  // Realtime subscription to current voice row
  const subscribeToRow = useCallback((rowId: string) => {
    stopRealtime();
  ...
  const pollValidate = useCallback((taskId: string) => {
    stopPolling();
