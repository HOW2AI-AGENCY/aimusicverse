...
const WEIGHTS = {
  ...
  balance: 0.10,          // Low balance = higher risk
  session_depth: 0.10,    // Shallow sessions = higher risk
};
...
function calculateChurnScore(metrics: {
  ...
  const trendScore = monthlyRate > 0 
    ? Math.max(0, (1 - weeklyRate / monthlyRate) * 100)
    : (metrics.generations_last_7d === 0 ? 80 : 0);
  ...
  // 4. Balance risk
  const balanceScore = metrics.current_balance <= 0 
    ? 100 
    : metrics.current_balance <= 5 
      ? 70 
      : metrics.current_balance <= 10 
        ? 40 
        : 0;
