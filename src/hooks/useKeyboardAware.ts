...
export function useKeyboardAware() {
  ...
  useEffect(() => {
    ...
    const handleViewportChange = () => {
      ...
      const currentKeyboardHeight = Math.max(0, window.innerHeight - viewport.height);
...
export function useKeyboardAwareContainer(
  ...
  useEffect(() => {
    ...
    if (autoScrollInputs) {
      const inputs = element.querySelectorAll<HTMLElement>('input, textarea, select, [contenteditable="true"]');
