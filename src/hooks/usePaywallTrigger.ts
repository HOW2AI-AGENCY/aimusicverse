...
export function usePaywallTrigger(options: UsePaywallTriggerOptions = {}) {
  ...
  const triggerPaywall = useCallback(
    (reason: PaywallTriggerReason): boolean => {
      if (hasSubscription) {
  ...
  const shouldTriggerForFeature = useCallback(
...
export function usePaywallAfterAction(actionType: "generation" | "feature_use") {
  const { generationCount, isFreeUser, markPaywallShown, isInCooldown } = usePaywallTrigger();
  ...
  const shouldShowAfterAction = useMemo(() => {
    ...
    if (actionType === "generation") {
      ...
      return generationCount === SOFT_TRIGGER_GENERATIONS || generationCount === HARD_TRIGGER_GENERATIONS;
    }
