...
export function useStudioChangeLog(trackId: string) {
  ...
  const logMutation = useMutation({
    mutationFn: async ({ changeType, options = {} }: { changeType: StudioChangeType; options?: LogChangeOptions }) => {
  ...
  const logSectionReplacementStart = useCallback(
    (sectionStart: number, sectionEnd: number, prompt?: string, tags?: string[]) => {
  ...
  const logSectionReplacementComplete = useCallback(
    (taskId: string, versionId: string, audioUrls: string[], sectionStart: number, sectionEnd: number) => {
      return logMutation.mutateAsync({
  ...
  const logSectionReplacementApplied = useCallback(
    (versionId: string, variant: "variantA" | "variantB") => {
      return logMutation.mutateAsync({
  ...
  const logStemSeparationStart = useCallback(
  ...
  const logStemSeparationComplete = useCallback(
    (stemTypes: string[], stemCount: number) => {
      return logMutation.mutateAsync({
  ...
  const logStemAction = useCallback(
    (stemId: string, stemType: string, action: "muted" | "unmuted" | "solo" | "volume_changed", value?: number) => {
  ...
  const logVersionSwitch = useCallback(
    (oldVersionId: string | null, newVersionId: string) => {
      return logMutation.mutateAsync({
  ...
  const logStudioSession = useCallback(
