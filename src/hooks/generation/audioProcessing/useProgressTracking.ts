...
import { STATUS_MESSAGES, STATUS_PROGRESS, DEFAULT_TITLES, INVALIDATE_QUERY_KEYS } from "./constants";
...
export function useProgressTracking(operation: AudioProcessingOperation): ProgressTrackingReturn {
   ...
   */
  const fetchCompletedTrack = useCallback(
    async (trackId: string): Promise<CompletedTrack | null> => {
  ...
  const handleTaskUpdate = useCallback(
    async (taskStatus: string, errorMessage?: string, taskTrackId?: string) => {
  ...
  const startTracking = useCallback(
    (taskId: string, trackId: string, extra?: { studioProjectId?: string }) => {
