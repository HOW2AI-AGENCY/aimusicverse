...
export function useAudioProcessing(): UseAudioProcessingReturn {
  ...
  const extend = useCallback(async (params: ExtendParams): Promise<OperationResult> => {
  ...
  const cover = useCallback(async (params: CoverParams): Promise<OperationResult> => {
  ...
  const addVocals = useCallback(async (params: AddVocalsParams): Promise<OperationResult> => {
  ...
  const addInstrumental = useCallback(async (params: AddInstrumentalParams): Promise<OperationResult> => {
    ...
    try {
      ...
      if (data?.taskId) {
        addInstrumentalProgress.startTracking(
          data.taskId, 
          data.trackId || data.taskId,
  ...
  const activeOperations = useMemo<AudioProcessingOperation[]>(() => {
    ...
    extendProgress.isActive, 
    coverProgress.isActive, 
    addVocalsProgress.isActive, 
    addInstrumentalProgress.isActive
  ]);
