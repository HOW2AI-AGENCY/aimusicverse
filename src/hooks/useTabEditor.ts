...
export function useTabEditor(options: UseTabEditorOptions = {}) {
  const { initialNotes = [], bpm = 120, onNotesChange } = options;
  ...
  const addNote = useCallback(
    (string: number, fret: number, position: number, duration: number = 1) => {
      const note: TabNote = {
        id: generateNoteId(),
        string,
        fret,
  ...
  const updateNote = useCallback(
    (id: string, updates: Partial<Omit<TabNote, "id">>) => {
      const newNotes = state.notes.map((note) => (note.id === id ? { ...note, ...updates } : note));
      saveToHistory(newNotes);
      haptic.tap();
    },
    [state.notes, saveToHistory, haptic],
  ...
  const applyTechnique = useCallback(
    (technique: TabNote["technique"]) => {
      if (state.selectedNotes.size === 0) return;

      const newNotes = state.notes.map((note) => (state.selectedNotes.has(note.id) ? { ...note, technique } : note));
      saveToHistory(newNotes);
      haptic.impact("medium");
    },
    [state.notes, state.selectedNotes, saveToHistory, haptic],
   ...
   */
  const getNotesAtPosition = useCallback(
    (position: number) => {
      return state.notes.filter((n) => n.position <= position && n.position + n.duration > position);
    },
    [state.notes],
  );
