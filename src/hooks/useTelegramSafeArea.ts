...
export function useTelegramSafeArea(options: UseTelegramSafeAreaOptions = {}): SafeAreaStyles {
  const { withBottomNav = false, withMainButton = true, extraTop = 0, extraBottom = 0 } = options;
...
export const safeAreaClasses = {
  ...
  safeHorizontal:
    "pl-[max(var(--tg-safe-area-inset-left,0px),env(safe-area-inset-left,0px))] pr-[max(var(--tg-safe-area-inset-right,0px),env(safe-area-inset-right,0px))]",
