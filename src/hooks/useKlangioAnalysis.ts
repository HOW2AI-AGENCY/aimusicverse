...
export function useKlangioAnalysis() {
  ...
  const startTranscription = useCallback(
    async (
      ...
      try {
        ...
        if (data?.error) {
          const errorMsg =
            data.error === "no_notes_found" ? "Не удалось распознать ноты в записи" : data.message || data.error;
  ...
  const detectChords = useCallback(
    async (audioUrl: string, extended: boolean = true): Promise<ChordResult | null> => {
      if (!audioUrl) {
        toast.error("Аудио URL не указан");
        return null;
      }
  ...
  const detectBeats = useCallback(
    async (audioUrl: string): Promise<BeatResult | null> => {
      if (!audioUrl) {
        toast.error("Аудио URL не указан");
        return null;
      }
