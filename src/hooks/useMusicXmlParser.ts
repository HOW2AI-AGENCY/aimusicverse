...
export function useMusicXmlParser() {
  ...
  const parseMusicXmlFromUrl = useCallback(
    async (xmlUrl: string): Promise<ParsedMusicXml | null> => {
      ...
      try {
        ...
        const title =
          xmlDoc.querySelector("work > work-title")?.textContent ||
        ...
        partElements.forEach((partEl) => {
          ...
          const partName =
            xmlDoc.querySelector(`part-list > score-part[id="${partId}"] > part-name`)?.textContent || partId;
