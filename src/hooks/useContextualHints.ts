...
export const CONTEXTUAL_HINTS: ContextualHint[] = [
  // Model hints
  {
    ...
    description: 'Suno V5 доступна для генерации! Она быстрее и создает более качественный звук. Откройте расширенные настройки при создании трека.',
    action: {
      label: 'Создать трек',
      route: '/?generate=true',
    },
  ...
  },
  {
    ...
    description: 'Разные модели подходят для разных задач: V5 для скорости, V4.5+ для богатого звука до 8 минут. Меняйте модель в Advanced Options.',
    showOnRoutes: ['/', '/library'],
    minVisits: 5,
  ...
  // AI Features
  {
    ...
    description: 'Создавайте профессиональные тексты песен с помощью AI! Помощник сам определит структуру, добавит теги и предложит улучшения. 18 быстрых действий для творчества!',
    action: {
      label: 'Попробовать',
      route: '/?generate=true',
    },
  ...
  },
  {
    ...
    description: 'Загрузите свою музыку для AI-анализа: определение аккордов, темпа, структуры композиции. Идеально для кавер-версий и ремиксов!',
  ...
  },
  {
    ...
    description: 'Разделите любой трек на отдельные дорожки: вокал, инструменты, бас, барабаны. Создавайте ремиксы и караоке версии!',
    action: {
  ...
  // AI Artists
  {
    ...
    description: 'AI артисты помогают создавать треки в узнаваемом стиле. Определите жанр, голос, визуальный стиль — и создавайте целые дискографии!',
    action: {
  ...
  },
  {
    ...
    description: 'Понравился стиль трека? Создайте AI артиста на его основе! Откройте меню трека → "Создать артиста из трека".',
    showOnRoutes: ['/library', '/'],
    minVisits: 5,
    cooldownHours: 96,
    maxShows: 2,
  ...
  // Projects
  {
    ...
    description: 'Создайте проект и распишите план треков с описанием каждого. AI поможет генерировать треки по очереди с учетом общей концепции альбома!',
    action: {
  ...
  },
  {
    ...
    description: 'В проектах доступны AI действия: генерация обложки, создание описания, анализ треклиста. Используйте AI для полного цикла работы над альбомом!',
  ...
  // Social Features
  {
    ...
    description: 'Публикуйте треки в сообществе, делитесь в Telegram Stories или отправляйте друзьям. Получайте лайки и комментарии!',
    showOnRoutes: ['/library', '/'],
    minVisits: 4,
    cooldownHours: 96,
    maxShows: 2,
  ...
  // Advanced Tips
  {
    ...
    description: 'В форме генерации доступны продвинутые настройки: референс аудио, вес стиля, negative tags, пол вокала. Экспериментируйте для точного результата!',
    showOnRoutes: ['/', '/library'],
    minVisits: 6,
    cooldownHours: 144,
    maxShows: 2,
  ...
  },
  {
    ...
    description: 'Используйте Space для паузы/воспроизведения, ← → для перемотки. Открывайте список горячих клавиш через "?" в плеере.',
    showOnRoutes: ['/library', '/'],
    minVisits: 7,
    cooldownHours: 168,
    maxShows: 1,
  ...
  },
  {
    ...
    description: 'Заходите каждый день, чтобы получать кредиты! Непрерывная серия посещений увеличивает награду. Смотрите прогресс в виджете геймификации.',
    showOnRoutes: ['/', '/library'],
    minVisits: 2,
    cooldownHours: 48,
    maxShows: 3,
  ...
  },
  {
    ...
    category: 'tip',
    title: '🔄 A/B версии',
    description: 'Каждая генерация создает 2 версии трека (A и B). Сравнивайте их и выбирайте лучшую! Переключайте версии через значок A/B.',
    showOnRoutes: ['/library'],
    minVisits: 3,
    cooldownHours: 96,
    maxShows: 2,
...
export function useContextualHints(currentRoute?: string) {
  ...
  const canShowHint = useCallback((hint: ContextualHint): boolean => {
  ...
  const getNextHint = useCallback((): ContextualHint | null => {
    const availableHints = CONTEXTUAL_HINTS
      .filter(canShowHint)
  ...
  // Show hint
  const showHint = useCallback((hint: ContextualHint) => {
    setCurrentHint(hint);
  ...
  const dismissHint = useCallback((forever: boolean = false) => {
