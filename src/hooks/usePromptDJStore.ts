...
export const usePromptDJStore = create<PromptDJState>()(
  persist(
    (set, get) => ({
      ...
      rateHistoryEntry: (id, rating) =>
        set((state) => ({
          promptHistory: state.promptHistory.map((entry) => (entry.id === id ? { ...entry, rating } : entry)),
        })),
...
export const selectTopRatedPrompts = (state: PromptDJState) =>
  state.promptHistory.filter((entry) => entry.rating && entry.rating >= 4).slice(0, 10);

export const selectRecentPrompts = (state: PromptDJState) => state.promptHistory.slice(0, 5);
