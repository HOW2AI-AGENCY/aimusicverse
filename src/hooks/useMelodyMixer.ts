...
export function useMelodyMixer(options: UseMelodyMixerOptions = {}) {
  ...
  const updateSlotWeight = useCallback((id: string, weight: number) => {
    setState((prev) => ({
      ...prev,
      slots: prev.slots.map((slot) => (slot.id === id ? { ...slot, weight: Math.max(0, Math.min(1, weight)) } : slot)),
    }));
  }, []);
  ...
  const toggleSlot = useCallback(
    (id: string) => {
      setState((prev) => ({
        ...prev,
        slots: prev.slots.map((slot) => (slot.id === id ? { ...slot, enabled: !slot.enabled } : slot)),
      }));
      haptic.tap();
    },
    [haptic],
  ...
  const updateSlotName = useCallback((id: string, name: string) => {
    setState((prev) => ({
      ...prev,
      slots: prev.slots.map((slot) => (slot.id === id ? { ...slot, name } : slot)),
    }));
  }, []);
  ...
  const getStylePrompt = useCallback(() => {
    const activeSlots = state.slots.filter((s) => s.enabled && s.weight > 0).sort((a, b) => b.weight - a.weight);

    if (activeSlots.length === 0) return "";

    return activeSlots.map((s) => `${s.name} (${Math.round(s.weight * 100)}%)`).join(", ");
  }, [state.slots]);
