...
import { ReferenceManager, UnifiedAudioReference, ReferenceMode, AudioAnalysis } from "@/services/audio-reference";
...
export function useAudioReference(): UseAudioReferenceReturn {
  ...
  // Set from recording
  const setFromRecording = useCallback(async (blob: Blob, fileName?: string, mode?: ReferenceMode) => {
    setIsLoading(true);
    try {
  ...
  const setFromCreativeTool = useCallback(
    (source: "drums" | "dj", audioUrl: string, options?: { prompt?: string; tags?: string; bpm?: number }) => {
  ...
  const setFromGuitar = useCallback(
    (data: {
      audioUrl: string;
      bpm?: number;
      chordProgression?: string[];
  ...
  const setFromTrack = useCallback(
    (
      track: {
        id: string;
        audioUrl: string;
  ...
  // Derived state
  const hasAnalysis = !!activeReference?.analysis && activeReference.analysisStatus === "completed";
