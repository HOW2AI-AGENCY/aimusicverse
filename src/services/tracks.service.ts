...
export async function fetchTracksWithLikes(
  ...
  // Fetch base tracks
  const response = await tracksApi.fetchTracks({ ...filters, userId }, pagination);
  ...
  const hasMore = pagination ? (pagination.page + 1) * pagination.pageSize < totalCount : false;
...
export async function fetchPublicTracksWithCreators(
  ...
  const response = await tracksApi.fetchTracks({ isPublic: true, sortBy: "recent" }, pagination);
 ...
 */
export async function updateTrackVisibility(trackId: string, isPublic: boolean): Promise<tracksApi.TrackRow> {
