    ...
    <AnimatePresence mode="wait">
      {isLoading ? (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className={cn("grid gap-4", isMobile ? "sm:grid-cols-2 lg:grid-cols-3" : "grid-cols-1 lg:grid-cols-2")}
        >
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <motion.div
              key={i}
                          ...
                          <div className="flex flex-wrap gap-1.5 mt-3">
                            {artist.genre_tags.slice(0, 3).map((tag: string) => (
                              <Badge
                                key={tag}
                                variant="secondary"
                                className="text-xs gap-1 bg-primary/10 text-primary border-0"
                              >
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="relative overflow-hidden border-b border-border/30 bg-gradient-to-b from-primary/5 to-background">
        <div
          className={cn("container mx-auto px-4 pb-4 relative", isMobile ? "max-w-6xl" : "max-w-7xl")}
          style={{
            paddingTop:
        ...
        >
          ...
          >
            <div className="flex items-center gap-2.5">
              <motion.div className="p-2 rounded-xl bg-gradient-to-br from-primary/20 to-primary/5 border border-primary/20">
      ...
      <div className={cn("container mx-auto px-4 py-4", isMobile ? "max-w-6xl" : "max-w-7xl")}>
        {/* Tabs */}
        ...
        <div className="flex gap-1.5 overflow-x-auto pb-3 mb-4 scrollbar-hide -mx-4 px-4">
          ...
          {GENRES.map((genre) => (
            <Badge
              ...
              className={cn(
                "cursor-pointer whitespace-nowrap px-2.5 py-1 text-xs",
        ...
        {isMobile ? (
          ...
          <div className="flex gap-6 min-h-[calc(100vh-300px)]">
            {/* Master List */}
            <div className={cn("transition-all duration-300", selectedArtist ? "w-[60%]" : "w-full")}>
              {renderArtistList()}
            </div>
            ...
            {!selectedArtist && currentArtists.length > 0 && (
              <div className="hidden lg:flex w-[40%] items-center justify-center border-l border-border/30 pl-6">
                <div className="text-center p-6">
                  ...
                  <p className="text-muted-foreground">Выберите артиста для просмотра</p>
                </div>
              </div>
            )}
          </div>
      ...
      <ArtistDetailsPanel artist={selectedArtist} open={detailsPanelOpen} onOpenChange={setDetailsPanelOpen} />
    </div>
  );
}
