...
export default function BuyCredits() {
  ...
  const tracksCount = selectedProduct 
    ? Math.floor((selectedProduct.credits_amount ?? 0) / 12) 
    : 0;
    ...
    <div className="container mx-auto px-4 py-6 max-w-4xl pb-40">
      ...
      >
        ...
        <p className="text-muted-foreground text-sm max-w-md mx-auto">
          Приобретайте кредиты для создания AI-музыки
        </p>
      </motion.div>
      ...
      <AnimatePresence>
        {showPurchasePanel && selectedProduct && (
          <motion.div
            ...
            style={{
              paddingBottom: 'calc(max(var(--tg-safe-area-inset-bottom, 0px), env(safe-area-inset-bottom, 0px)) + 4.5rem)'
            }}
          >
            <div className={cn(
              ...
              <div className={cn(
                "p-4 rounded-t-2xl border-t border-x",
                glass.overlay,
                "shadow-2xl"
              )}>
                  ...
                  >
                    {isProcessing ? (
                      ...
                      <>
                        ...
                        <span>Оплатить {selectedProduct.price_rub_cents ? formatRubles(selectedProduct.price_rub_cents) : ''}</span>
                ...
                {!selectedProduct.price_rub_cents && (
                  <p className="text-xs text-center text-destructive mt-2">
                    Цена не указана для этого пакета
                  </p>
                )}
              </div>
            </div>
