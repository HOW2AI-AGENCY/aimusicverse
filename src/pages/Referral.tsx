 ...
 * Desktop: Two-column layout (share + leaderboard side by side)
 */
...
import { Users, Gift, Share2, Copy, QrCode, Trophy, Sparkles, Crown, Medal, Link2 } from "lucide-react";
...
export default function Referral() {
  ...
  const renderShareSection = () => (
    <div className="space-y-4">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }}>
        <Card>
          <CardContent className="p-6 flex flex-col items-center space-y-4">
            ...
            <p className="text-sm text-muted-foreground text-center">Покажи QR-код другу или отправь ссылку</p>
          </CardContent>
        </Card>
      </motion.div>
          ...
          <CardContent className="space-y-3">
            <div className="flex items-center gap-2">
              <div className="flex-1 bg-muted rounded-lg px-4 py-3 font-mono text-lg text-center font-bold tracking-wider">
                {statsLoading ? <Skeleton className="h-6 w-24 mx-auto" /> : stats?.referralCode || "---"}
              </div>
              <Button variant="outline" size="icon" onClick={handleCopyCode} className="h-12 w-12">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
        <Button onClick={handleShare} className="w-full h-14 text-base font-semibold gap-2" size="lg">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex items-start gap-3">
              ...
              <p className="text-sm text-muted-foreground">
                Друг регистрируется и получает{" "}
                <span className="font-semibold text-foreground">{ECONOMY.REFERRAL_NEW_USER_BONUS}</span> кредитов
              </p>
            </div>
            <div className="flex items-start gap-3">
              <div className="w-6 h-6 rounded-full bg-primary/20 flex items-center justify-center text-xs font-bold text-primary flex-shrink-0">
              ...
              <p className="text-sm text-muted-foreground">
                Ты получаешь <span className="font-semibold text-foreground">{ECONOMY.REFERRAL_INVITE_BONUS}</span>{" "}
                кредитов после регистрации
              </p>
            </div>
          </CardContent>
        </Card>
  ...
  const renderLeaderboardSection = () => (
    <Card className="h-full">
      <CardHeader className="pb-2">
        ...
        <CardDescription>Пользователи с наибольшим количеством приглашений</CardDescription>
      </CardHeader>
      <CardContent>
        <ScrollArea className={cn("pr-2", isMobile ? "h-[400px]" : "h-[500px]")}>
          {leaderboardLoading ? (
  ...
  return (
    <div className="min-h-screen pb-24">
      <div className={cn("container mx-auto px-4 py-6", isMobile ? "max-w-lg" : "max-w-5xl")}>
        {/* Header */}
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <p className="text-muted-foreground text-sm">
            Получай <span className="text-primary font-semibold">{ECONOMY.REFERRAL_INVITE_BONUS}</span> кредитов за
            каждого друга
          </p>
        </motion.div>
        ...
        {/* Desktop: Two columns | Mobile: Stacked tabs */}
        {isMobile ? (
