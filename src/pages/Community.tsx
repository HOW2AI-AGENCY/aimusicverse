...
import { Globe, Music, Users, TrendingUp, Heart, Search, X, RefreshCw, Sparkles, Compass } from "@/lib/icons";
...
const GridContainer = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ children, style, ...props }, ref) => (
    <div ref={ref} style={style} {...props} className={cn("grid gap-3", props.className)}>
      {children}
    </div>
  ),
);
...
const ListContainer = forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ children, style, ...props }, ref) => (
    <div ref={ref} style={style} {...props} className="flex flex-col gap-3">
      {children}
    </div>
  ),
);
                ...
                viewMode === "grid" ? (
                  <VirtuosoGrid
                    ...
                    itemContent={(index, track) => (
                      <UnifiedTrackCard key={track.id} variant="enhanced" track={track} compact={false} />
                    )}
                  />
                ) : (
                  <VirtuosoGrid
