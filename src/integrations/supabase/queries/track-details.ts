...
export async function fetchTrackDetails({
  ...
  if (includeStems) {
    ...
    const stemsRes = await supabase.from("track_stems").select("*").eq("track_id", trackId).order("stem_type");
    if (stemsRes.error) throw stemsRes.error;
    response.stems = stemsRes.data;
  }

  if (includeAnalysis) {
    const analysisRes = await supabase.from("audio_analysis").select("*").eq("track_id", trackId).maybeSingle();
...
export async function fetchTrackStems(trackId: string) {
  const { data, error } = await supabase.from("track_stems").select("*").eq("track_id", trackId).order("stem_type");
...
export async function fetchTrackAnalysis(trackId: string) {
  const { data, error } = await supabase.from("audio_analysis").select("*").eq("track_id", trackId).maybeSingle();
...
export async function setPrimaryVersion(versionId: string, trackId: string) {
  ...
  // Set new primary
  const { error: setError } = await supabase.from("track_versions").update({ is_primary: true }).eq("id", versionId);
