...
| 055-B3 | **Hint позиционирование**: перенести первый-hint в absolute relative to FAB с offset `-top-12 -translate-y-full`, чтобы не перекрывал контент выше. Storybook story для `BottomNavigation` hint                                | 🟡 Medium | 0.2d   | ⏭ Skip — current positioning works correctly with safe-area handling |
