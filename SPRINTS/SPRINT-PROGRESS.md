...
| **Sprint 052: Suno API Mashup + Persona + File Upload** | 🔄 В РАБОТЕ     | **A1–A5** ✅ edge `suno-mashup/persona/file-upload` + DB `track_personas` + cover/extend → shared uploader + **editorial layout `LyricsVisualEditorCompact` + `SectionTagSelector`** (`916cd72a`, `a9d12426`, `37ca8264`, `18b1e80e`). **B1** ⏳ UI `MashupDialog` + hooks. **B2** ⏳ Telegram `/mashup`. Δvalue: viral UGC loop. План: [SPRINT-052-PLAN.md](./SPRINT-052-PLAN.md)                                             |
