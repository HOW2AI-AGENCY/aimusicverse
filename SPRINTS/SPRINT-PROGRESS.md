...
| **Sprint 052: Suno API Mashup + Persona + File Upload** | ⏳ ЗАПЛАНИРОВАН | Закрывает 3 gap-категории Suno API. Edge: `suno-mashup`, `suno-mashup-callback`, `suno-persona`, `suno-persona-callback`, `suno-file-upload`. DB: `track_personas` + `track_versions.persona_id`. UI: `MashupDialog`, `useSunoMashup`, `useSunoPersona`, `useSunoFileUpload`. Telegram: `/mashup`. Δvalue: viral UGC loop, дифференциатор. Детальный план: [SPRINTS/SPRINT-052-PLAN.md](./SPRINT-052-PLAN.md)                  |
