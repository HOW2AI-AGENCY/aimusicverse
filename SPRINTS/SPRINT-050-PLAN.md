...
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ------ |
| 050-A1 | **E2E вердикт**: дождаться/перезапустить run #115; по фактическим падениям тестов — чинить (сбор suite уже исправлен в #572)                                                                                                                                                                                                                             | 🔴 Critical | 1–3d   |
| 050-A2 | **Docs/lychee**: ✅ частично закрыто follow-up PR после #573 (exclude-path для generated/архивных каталогов, ~10 битых ссылок в живых доках починено). Осталось: решить судьбу 4434 закоммиченных файлов `graphify-out/**` (`.gitignore` уже объявляет каталог игнорируемым — либо `git rm -r --cached`, либо признать tracked и убрать из `.gitignore`) | 🟠 High     | 0.5d   |
| 050-A3 | **Сверка прод-миграций**: `supabase migration list` (или MCP `list_migrations` с approval) vs `supabase/migrations/`; проверить последовательную накатку обеих likes-миграций (`20260703120000` + `20260704014859`) на локальном стеке                                                                                                                   | 🟠 High     | 0.5d   |
| 050-A4 | **Процесс Lovable**: выбрать и внедрить один из вариантов — (а) auto-format workflow на push в `main` (bot-коммит `prettier --write`), (б) branch protection + Lovable работает через ветку/PR. Решение согласовать с владельцем                                                                                                                         | 🔴 Critical | 1d     |
| 050-A5 | **Гигиена репозитория**: решить судьбу `bun.lock`/`bun.lockb` vs `package-lock.json` (один пакетный менеджер как источник истины; зависит от 050-A4)                                                                                                                                                                                                     | 🟡 Medium   | 0.5d   |

## Фаза B: F1–F12 + функциональные переносы (дни 6–10)

Флаги из CHANGELOG (спринты 045–047), сгруппированы по поверхностям:

| ID     | Задача                                                                                                                                | Флаги        | Оценка |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------- | ------------ | ------ |
| 050-B1 | **Scroll/viewport**: `useScrollLock` wiring в 4 surfaces; `useVisualViewport` keyboard avoidance; `useMediaQuery` SSR-guard           | F1, F8, F9   | 1.5d   |
| 050-B2 | **Focus/keyboard**: focus-trap в sheet/dialog; Library keyboard nav (`aria-selected` + scroll-into-view); Telegram BackButton race    | F3, F6, F7   | 1.5d   |
| 050-B3 | **Поверхности**: `PromptHistory` sub-dialog root cause; QueueSheet auto-close + misleading toast copy; `LibraryFilterChips` parity    | F2, F5, F11  | 1d     |
| 050-B4 | **Данные/чистка**: `usePublicTracks` cover_url normalization; `VersionComparison.tsx` dead-code removal; `LazyImage` a11y             | F4, F10, F12 | 1d     |
| 050-B5 | **ErrorBoundary home button** (`useNavigate`) — перенос из Sprint 045 Phase D-4                                                       | —            | 0.5d   |
| 050-B6 | **Bundle quick wins**: `lamejs` (~100 КБ), `canvas-confetti` (~20 КБ), `qrcode` (~30 КБ) — динамический импорт, вон из `vendor-other` | —            | 0.5d   |
