...
export function fromSupabase<T>(result: { data: T | null; error: PostgrestError | null }): Result<T> {
...
self.addEventListener("install", (event) => {
  event.waitUntil(
    caches
      .open("musicverse-v1")
      .then((cache) => cache.addAll(["/", "/index.html", "/assets/vendor-react.js", "/assets/vendor-tone.js"])),
  );
});
