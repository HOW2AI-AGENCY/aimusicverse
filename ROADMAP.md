...
- **Sprint 050 — Main Green + Mobile Audit F1–F12** ([детальный план](SPRINTS/SPRINT-050-PLAN.md)). **Phase A ✅ завершена (6/6).** **Phase B 🔄 5/6 done:** B1 ✅ `useScrollLock` на 4 surfaces, B3 ✅ QueueSheet auto-close + toast, B4 ✅ cover_url normalization, B5 ✅ ErrorBoundary, B6 ✅ lazy-imports (lamejs/qrcode/canvas-confetti). Remaining: A4 Phase 2 (required checks — manual), E2E verdict, Docs (GitHub Pages). PR #615 + #616 awaiting CI.
...
- **Sprint 052 — Suno API: Mashup + Persona + File Upload** ([детальный план](SPRINTS/SPRINT-052-PLAN.md)). Закрывает 3 gap-категории Suno API (mashup двух треков, голосовые персоны, прокси загрузки файлов). 5 новых edge (`suno-mashup`, `suno-mashup-callback`, `suno-persona`, `suno-persona-callback`, `suno-file-upload`), DB-миграция `track_personas` + `track_versions.persona_id`, рефакторинг `suno-upload-cover/extend` на общий `suno-file-upload`. UI: `MashupDialog`, `useSunoMashup`, `useSunoPersona`, `useSunoFileUpload`, кнопка «Create Persona» в `GenerationResultSheet`. Telegram: `/mashup`. Δvalue: viral UGC loop, дифференциатор от конкурентов. ✅ **Завершён 2026-07-05** (10/10 + cleanup ✅). Sprint 052-C cleanup: Storybook stories + i18n strings (commit `93beb2f1`). Commits `916cd72a` … `93beb2f1`. E2E: `tests/e2e/suno-mashup.spec.ts`.
- **Sprint 053 — Suno API: Sounds + MIDI Direct + Boost Style** ([детальный план](SPRINTS/SPRINT-053-PLAN.md)). Закрывает 3 gap-категории: нативный Suno SFX с `tempo/key/duration` (заменяет Replicate в `generate-sfx`), Suno MIDI direct (заменяет Replicate в `transcribe-midi` для Studio с graceful fallback), подключение к UI существующего `suno-boost-style`. 5 новых edge + 1 рефакторинг. UI: `SfxGeneratorSheet` в Library, MIDI-fallback в Studio, Boost Style menu item. Telegram: `/sfx`. Δvalue: новый сегмент SFX + снижение Replicate-зависимости.
- **Sprint 054 — Suno API: Details Suite + Per-Task Introspection** ([детальный план](SPRINTS/SPRINT-054-PLAN.md)). Закрывает последний gap: 6 специализированных details-эндпоинтов Suno (`music/cover/video/wav/lyrics/separation/details`). Заменяет общий `suno-check-status` на диспетчер по `taskType` с per-type backoff. UI: generic `useSunoTaskDetails` заменяет 6 ad-hoc polling. Цель: снижение polling error-rate <2%.
- **Sprint 055 — Cost Optimization + Observability** (черновик): расширенный кредит-дашборд, retry-policy по типу таска, webhook-secret rotation, метрики latency per task_type в Sentry, hard-cap prompt/style/title по моделям V4/V4_5/V5 в `_shared/suno-validate.ts`.
...
- **Sprint 050 — Main Green + Mobile Audit F1–F12** 🔄 Phase B: 5/6 done. PR #615 (fix duplicate mcpPlugin) + PR #616 (Phase B: scroll lock, queue auto-close, bundle docs). Remaining blockers: A4 Phase 2 (required checks — manual GitHub UI), E2E verdict, Docs workflow (GitHub Pages enable).
