...
function validateDatabaseSchema() {
  ...
  if (existsSync(trackVersionsMigration)) {
    ...
    if (content.includes("CREATE TABLE public.track_versions")) {
      addResult("Database Schema", "track_versions table", "PASS", "track_versions table migration exists");
    } else {
      addResult("Database Schema", "track_versions table", "FAIL", "track_versions table not found in migration");
    }

    if (content.includes("is_primary")) {
      addResult("Database Schema", "is_primary field", "PASS", "is_primary field found in track_versions schema");
    } else {
      addResult("Database Schema", "is_primary field", "WARN", "is_primary field not explicitly found in migration");
    }
  } else {
    addResult("Database Schema", "track_versions migration", "FAIL", "track_versions migration file not found");
  }
  ...
  if (existsSync(changelogMigration)) {
    ...
    if (content.includes("CREATE TABLE public.track_change_log")) {
      addResult("Database Schema", "track_change_log table", "PASS", "track_change_log table migration exists");
    }
  } else {
    addResult("Database Schema", "track_change_log migration", "WARN", "track_change_log migration not found");
  }
...
function validateCodeStructure() {
  ...
  if (existsSync(versioningFile)) {
    ...
    if (content.includes("getVersionIndex")) {
      addResult("Code Structure", "getVersionIndex function", "PASS", "getVersionIndex function exists");
    }
    ...
    addResult("Code Structure", "versioning.ts file", "FAIL", "versioning.ts file not found");
  }
  ...
  if (existsSync(storageFile)) {
    ...
    addResult("Code Structure", "storage.ts file", "FAIL", "storage.ts file not found");
  }
  ...
  if (existsSync(cdnFile)) {
    ...
    if (content.includes("getCDNUrl") && content.includes("getOptimizedImageUrl")) {
      addResult("Code Structure", "cdn.ts helper functions", "PASS", "cdn.ts contains CDN and optimization functions");
    }

    if (content.includes("getResponsiveImageSrcSet")) {
      addResult("Code Structure", "cdn.ts responsive images", "PASS", "cdn.ts includes responsive image support");
    }
  } else {
    addResult("Code Structure", "cdn.ts file", "FAIL", "cdn.ts file not found");
  }
...
function validateDocumentation() {
  ...
  docsToCheck.forEach((doc) => {
    if (existsSync(doc)) {
      addResult("Documentation", doc, "PASS", `Documentation file ${doc} exists`);
    } else {
      addResult("Documentation", doc, "WARN", `Documentation file ${doc} not found`);
    }
  });
  ...
  if (existsSync(auditFile)) {
    ...
    if (content.includes("is_primary")) {
      addResult("Documentation", "Audit documents is_primary", "PASS", "Audit correctly documents is_primary field");
    }
...
function validateTelegramIntegration() {
  ...
  if (existsSync(telegramShareFile)) {
    ...
    if (content.includes("try") && content.includes("catch")) {
      addResult("Telegram Integration", "Error handling", "PASS", "telegram-share.ts includes error handling");
    }
  } else {
    addResult("Telegram Integration", "telegram-share.ts file", "WARN", "telegram-share.ts file not found");
  }
}
