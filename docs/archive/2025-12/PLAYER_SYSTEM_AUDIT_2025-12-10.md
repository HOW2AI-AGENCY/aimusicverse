...
const validAudios = audios.filter(
  (audio) => audio.duration > 0 && !audio.error && audio.readyState >= 2, // HAVE_CURRENT_DATA or better
);
...
// After: Filter valid audios first
const validAudios = audios.filter((audio) => audio.duration > 0 && !audio.error && audio.readyState >= 2);
