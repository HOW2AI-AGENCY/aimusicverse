...
| Всего JS (все чанки, gzip)          |  **2.11 МБ**  |  ≤ 950 КБ¹ |   🟡   |
...
<sub>¹ `size-limit`'s 950 КБ порог исторически считался по всему `dist/assets/*.js` (2.11 МБ сейчас) — эта метрика включает admin/studio/lazy-чанки, которые не грузятся на обычной странице. Реальный вес того, что скачивает пользователь на холодном старте — 508 КБ gzip, см. [docs/BUNDLE_ANALYSIS.md](docs/BUNDLE_ANALYSIS.md).</sub>
...
| Total bundle (все чанки) 2.11 МБ / 950 КБ size-limit порог | 🟡 Medium | follow-up (не блокирует, eager-load уже 508 КБ) |
...
<sub>Последнее обновление: 2026-07-03 (#568 eager-load бандл 1.19 МБ → 508 КБ, #567 any-cleanup 0/342, #566/#562 редизайн карточек треков + homepage reconnect, Sprint 049 ✅ — Mobile UX: залипание скролла + жанровые секции, A/B версии обновляют карточку, per-version лайки, пейджер/лирика полноэкранного плеера)</sub>
